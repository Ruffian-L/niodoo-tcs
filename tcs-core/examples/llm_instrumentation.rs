...
pub fn instrumented_llm_prompt(prompt: &str, output: &str) -> Result<String, String> {
    init_metrics();
    let registry = get_registry();

    let llm_counter = registry
        .get_metric("tcs_llm_prompts_total")
...
fn calculate_variance(text: &str) -> f64 {
    ...
    let variance = chars.iter().map(|&x| (x - mean).powi(2)).sum::<f64>() / chars.len() as f64;
