    ...
    pub fn from_env() -> Result<Self> {
        ...
        let huggingface_cache_dir = if let Ok(hf_cache) = env::var("HF_HOME") {
            ...
            if let Some(home_path) = home {
                PathBuf::from(home_path).join(".cache/huggingface")
            } else {
                std::env::temp_dir().join(".cache/huggingface")
            }
        };
...
pub struct ModelConfig {
    ...
    pub num_heads: usize,
    /// Qwen runtime configuration for CUDA and model loading
    pub qwen_runtime: QwenRuntimeConfig,
}
...
pub struct QwenRuntimeConfig {
    /// CUDA compute capability (e.g., "8.6" for RTX 3000/4000 series)
    pub cuda_compute_cap: String,
...
impl Default for QwenRuntimeConfig {
    fn default() -> Self {
        Self {
            cuda_compute_cap: env_var_with_default("CUDA_COMPUTE_CAP", "8.6"),
            cuda_arch: env_var_with_default("CUDA_ARCH", "sm_86"),
            use_cuda: env_var_with_default("QWEN_USE_CUDA", "true")
                .parse()
                .unwrap_or(true),
            model_dir: env_var_with_default(
                "QWEN_MODEL_DIR",
                &dirs::home_dir()
            ...
            tokenizer_path: env_var_with_default(
                "QWEN_TOKENIZER_PATH",
                &dirs::home_dir()
                    .unwrap_or_else(|| std::path::PathBuf::from("/tmp"))
                    .join("models/Qwen2.5-7B-Instruct-AWQ/tokenizer.json")
                    .to_string_lossy()
                    .to_string(),
            ),
            seed: env_var_with_default("QWEN_SEED", "42").parse().ok(),
        }
    }
}
...
pub struct ConsciousnessConfig {
    ...
    /// Enable dynamic token promotion
    #[serde(default = "default_token_promotion_enabled")]
    pub token_promotion_enabled: bool,
    /// Interval between promotion cycles (seconds)
    #[serde(default = "default_token_promotion_interval")]
    pub token_promotion_interval: u64,
    /// Minimum promotion score to consider a candidate token
    #[serde(default = "default_token_promotion_min_score")]
    pub token_promotion_min_score: f64,
    /// Maximum number of candidate tokens examined per cycle
    #[serde(default = "default_token_promotion_max_per_cycle")]
    pub token_promotion_max_per_cycle: usize,
    /// Consensus threshold for accepting token promotions
    #[serde(default = "default_token_promotion_consensus_threshold")]
    pub token_promotion_consensus_threshold: f64,
    /// Minimum usage count before pruning promoted tokens
    #[serde(default = "default_token_promotion_pruning_min_usage")]
    pub token_promotion_pruning_min_usage: usize,
    /// Path to tokenizer JSON used for dynamic promotion state
    #[serde(default = "default_tokenizer_path")]
    pub tokenizer_path: String,
}

fn default_token_promotion_enabled() -> bool {
    true
}
...
fn default_token_promotion_min_score() -> f64 {
    0.5 // Lowered from 0.75 to allow more tokens to be promoted
}

fn default_token_promotion_max_per_cycle() -> usize {
    50 // Set to 50 per cycle as specified
}

fn default_token_promotion_consensus_threshold() -> f64 {
    0.66
}

fn default_token_promotion_pruning_min_usage() -> usize {
    10
}
...
impl Default for ModelConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            num_heads: 32,
            qwen_runtime: QwenRuntimeConfig::default(),
        }
    }
}
...
impl Default for ConsciousnessConfig {
    fn default() -> Self {
        Self {
            ...
            token_promotion_enabled: default_token_promotion_enabled(),
            token_promotion_interval: default_token_promotion_interval(),
            token_promotion_min_score: default_token_promotion_min_score(),
            token_promotion_max_per_cycle: default_token_promotion_max_per_cycle(),
            token_promotion_consensus_threshold: default_token_promotion_consensus_threshold(),
            token_promotion_pruning_min_usage: default_token_promotion_pruning_min_usage(),
...
impl AppConfig {
    ...
    fn validate(&mut self) -> Result<()> {
        ...
        // Relaxed tolerance to account for float rounding in serialization/deserialization
...
mod tests {
    ...
    fn test_config_with_new_sections() {
        ...
        // Set weights that sum to 1.0 to avoid validation issues with float rounding
        config.tracker.weights.satisfaction_factor_weight = 0.4;
        config.tracker.weights.emotion_stability_weight = 0.35;
        config.tracker.weights.history_consistency_weight = 0.25;
        ...
        // Use approximate equality for float comparison after serialization
        assert!((loaded.tracker.weights.satisfaction_factor_weight - 0.4).abs() < 0.01);
        assert!((loaded.tracker.weights.emotion_stability_weight - 0.35).abs() < 0.01);
        assert!((loaded.tracker.weights.history_consistency_weight - 0.25).abs() < 0.01);
