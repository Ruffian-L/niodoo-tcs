...
impl TokenCandidate {
    ...
    /// Gamma (emotional coherence weight) is tied to >0.3 PAD coherence threshold.
    pub fn promotion_score(&self) -> f64 {
        const ALPHA: f64 = 0.5; // Topological persistence
        const BETA: f64 = 0.3; // Usage frequency
        const GAMMA_BASE: f64 = 0.2; // Base emotional coherence weight
        const GAMMA_BOOST: f64 = 0.15; // Additional weight when coherence > 0.3
        const PAD_COHERENCE_THRESHOLD: f64 = 0.3; // Minimum emotional coherence for boost
        ...
        // Dynamic gamma: boost weight when emotional coherence exceeds threshold
        let gamma = if self.emotional_coherence > PAD_COHERENCE_THRESHOLD {
            GAMMA_BASE + GAMMA_BOOST
        } else {
            GAMMA_BASE
        };
