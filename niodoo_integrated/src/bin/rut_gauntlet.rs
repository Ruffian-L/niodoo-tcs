...
use niodoo_integrated::{
    types::{NiodooIntegrated, DynamicThresholds, EmotionalSample},
...
async fn run_100_prompt_test() -> Result<()> {
    ...
    let _ = settings.merge(config::File::with_name("rut_gauntlet_config"));
    let config: RutGauntletConfig = settings.get::<RutGauntletConfig>("rut_gauntlet").unwrap_or(RutGauntletConfig {
    ...
    for (i, prompt) in prompts.iter().enumerate().take(20) {
        ...
        let result = pipeline.process_pipeline(prompt).await?;
        let latency = start_time.elapsed().as_millis() as f64;
        let coherence = calculate_jaccard_similarity(&result.response, prompt);
        ...
        let test_result = TestResult {
            ...
            latency_ms: latency,
            learning_events: result.learning_events.clone(),
        ...
        breakthroughs += result.learning_events.iter().filter(|e| e.contains("breakthrough")).count();

        println!("Cycle {}/20: H={:.2}, Threat={}, Healing={}, Latency={:.0}ms", i+1, result.entropy, result.is_threat, result.is_healing, latency);
    }
    ...
    for (i, prompt) in prompts.iter().enumerate().skip(20) {
        ...
        if (i + 1) % 20 == 0 {
            println!("Cycle {}/100: H={:.2}, Threats={}, Healings={}, Latency={:.0}ms", i+1, result.entropy, 
