...
impl TrainingServiceServer {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        info!(port = self.port, "Starting training service server on {}", addr);
...
async fn submit_job_handler(
    ...
    let samples_vec: Vec<Vec<f32>> = request.samples
        .into_iter()
        .flat_map(|s| vec![s.input, s.target])
        .collect();

    let job = TrainingJob::new_rust(
        samples_vec,
        request.epochs,
...
async fn get_job_handler(
    ...
    let job = state
        .job_queue
        .get(&job_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to get job: {}", e)))?
...
async fn list_adapters_handler(
    ...
    let versions = state
        .adapter_storage
        .list_versions()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to list adapters: {}", e)))?;
...
async fn get_latest_adapter_handler(
    ...
    let latest_path = state
        .adapter_storage
        .get_latest()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to get latest adapter: {}", e)))?
