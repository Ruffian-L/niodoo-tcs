...
impl AdapterStorage {
    pub fn new(storage_base: impl AsRef<Path>) -> Result<Self> {
        ...
        fs::create_dir_all(&storage_base)
            .with_context(|| format!("Failed to create storage directory: {}", storage_base.display()))?;
    ...
    fn versioned_path(&self, timestamp: DateTime<Utc>) -> PathBuf {
        ...
        self.storage_base.join(format!("system2_adapters_v{}", version))
    }
    ...
    pub fn save_adapter(
        ...
        fs::create_dir_all(&versioned_path)
            .with_context(|| format!("Failed to create versioned directory: {}", versioned_path.display()))?;

        // Copy adapter files to versioned directory
        if adapter_path.is_dir() {
            // Python adapter (directory with multiple files)
            copy_dir_all(adapter_path, &versioned_path)
                .with_context(|| format!("Failed to copy adapter directory: {}", adapter_path.display()))?;
        } else {
            // Rust adapter (single safetensors file)
            let dest_file = versioned_path.join(adapter_path.file_name().unwrap_or_default());
            fs::copy(adapter_path, &dest_file)
                .with_context(|| format!("Failed to copy adapter file: {}", adapter_path.display()))?;
        }
    ...
    fn update_latest_symlink(&self, target: &Path) -> Result<()> {
        ...
        fs::create_dir_all(symlink_dir)
            .with_context(|| format!("Failed to create symlink directory: {}", symlink_dir.display()))?;

        // Remove existing symlink if it exists
        if symlink.exists() || symlink.is_symlink() {
            fs::remove_file(&symlink)
                .with_context(|| format!("Failed to remove existing symlink: {}", symlink.display()))?;
        }
        ...
        #[cfg(windows)]
        {
            ...
            if target.is_dir() {
                ...
                fs::write(&symlink, target.to_string_lossy().as_ref())
                    .with_context(|| format!("Failed to create symlink reference: {}", symlink.display()))?;
            } else {
                fs::copy(target, &symlink)
                    .with_context(|| format!("Failed to copy for symlink: {}", symlink.display()))?;
            }
        }
    ...
    pub fn list_versions(&self) -> Result<Vec<AdapterVersion>> {
        let entries = fs::read_dir(&self.storage_base)
            .with_context(|| format!("Failed to read storage directory: {}", self.storage_base.display()))?;
        ...
        for entry in entries {
            ...
            let metadata: AdapterMetadata = serde_json::from_str(&metadata_content)
                .with_context(|| format!("Failed to deserialize metadata: {}", metadata_path.display()))?;
        ...
        .with_context(|| format!("Failed to read source directory: {}", src.display()))?
    {
        ...
        if path.is_dir() {
            ...
            fs::copy(&path, &dst_path)
                .with_context(|| format!("Failed to copy file: {} -> {}", path.display(), dst_path.display()))?;
        }
    }
