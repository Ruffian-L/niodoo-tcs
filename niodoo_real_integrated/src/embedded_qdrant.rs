...
pub async fn spawn_embedded_qdrant() -> Result<tokio::process::Child> {
    let storage_path = std::env::var("QDRANT_STORAGE_PATH")
        .unwrap_or_else(|_| "./qdrant_storage".to_string());
    
    // Ensure storage directory exists
    std::fs::create_dir_all(&storage_path)
        .with_context(|| format!("failed to create Qdrant storage directory: {}", storage_path))?;
    ...
    cmd.args(&[
        ...
        "--grpc-port", "6334",  // Use different port for embedded
        "--http-port", "6333",  // Standard HTTP port
    ]);
    ...
    for attempt in 0..10 {
        if let Ok(resp) = client.get("http://127.0.0.1:6333/health").timeout(std::time::Duration::from_secs(1)).send().await {
