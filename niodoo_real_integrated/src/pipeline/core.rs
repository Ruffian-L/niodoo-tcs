...
/// **INTENTIONAL STUB**: This module is a placeholder for generated proto code.
/// 
/// The actual proto code should be generated by `build.rs` during compilation.
/// This stub provides minimal types needed for compilation until the build system
/// is fully configured.
/// 
/// **Status**: Not used in production - requires build.rs configuration.
#[allow(dead_code)]
pub mod proto {
...
pub struct Pipeline {
    ...
    pub(crate) config_arc: Arc<AsyncRwLock<RuntimeConfig>>,
    pub args: CliArgs,
    ...
    pub(crate) mcts_daydreamer: Arc<MctsDaydreamer>,
    ...
    pub(crate) sandbox_manager: Option<Arc<crate::sandbox::SandboxManager>>,
    pub(crate) revision_loop: Option<Arc<crate::constitutional::RevisionLoop>>,
    pub(crate) code_topology_analyzer: Arc<crate::code_topology::CodeTopologyAnalyzer>,
    // Phase 4: Fused Agent for Slow/Fast integration
    pub(crate) fused_agent: Option<Arc<crate::fused_agent::FusedAgent>>,
}
...
impl Pipeline {
    ...
    async fn initialise_with_topology(
        ...
        // Create Arc for code mode components (before moving generator)
        // Clone generator BEFORE wrapping in Arc to avoid move issues
        let generator_arc_for_code_mode = if config.code_mode.enabled {
            Some(Arc::new(generator.clone()))
        } else {
            None
        };
        ...
        // These return Option, so we check if initialization succeeded
        if crate::metrics::metrics().is_none() {
            warn!("Failed to initialize Prometheus metrics");
        }
        if crate::metrics::weighted_memory_metrics().is_none() {
            warn!("Failed to initialize weighted memory metrics");
        }
        ...
        Ok(Self {
            ...
            // Code Mode components
            sandbox_manager: if config.code_mode.enabled {
                let security_policy = crate::sandbox::SecurityPolicy {
                    allowed_imports: vec!["niodoo".to_string(), "numpy".to_string()],
                    timeout: Duration::from_secs(config.code_mode.sandbox_timeout_secs),
                    max_memory_bytes: Some(100 * 1024 * 1024), // 100 MB
                    max_code_length: config.code_mode.max_code_length,
                    allow_filesystem: false,
                    allow_network: false,
                };
                match crate::sandbox::SandboxManager::new(security_policy) {
                    Ok(manager) => {
                        info!("Sandbox manager initialized for code mode");
                        Some(Arc::new(manager))
                    }
                    Err(e) => {
                        warn!(%e, "Failed to initialize sandbox manager; code mode disabled");
                        None
                    }
                }
            } else {
            ...
            revision_loop: if config.code_mode.enabled && config.code_mode.constitutional_ai_enabled {
                if let Some(ref generator_arc) = generator_arc_for_code_mode {
                    let constitution = crate::constitutional::Constitution::default();
                    info!("Revision loop initialized for code mode");
                    Some(Arc::new(crate::constitutional::RevisionLoop::new(
                        generator_arc.clone(),
                        constitution,
                        config.code_mode.max_revision_attempts,
                    )))
                } else {
                    warn!("Generator Arc not available for revision loop; code mode disabled");
                    None
                }
            } else {
                None
            },
            code_topology_analyzer: Arc::new(crate::code_topology::CodeTopologyAnalyzer::new()),
            // Phase 4: Fused Agent for Slow/Fast integration
            fused_agent: if config.code_mode.enabled {
                let tcs_for_fused = if matches!(config.topology_mode, TopologyMode::Hybrid) {
                    match TCSAnalyzer::new_with_config(config.use_approximate_tda) {
                        Ok(analyzer) => Some(Arc::new(tokio::sync::Mutex::new(analyzer))),
                        Err(_) => None,
                    }
                } else {
                    None
                };
                // Reuse the same Arc we created for revision_loop
                if let Some(ref generator_arc) = generator_arc_for_code_mode {
                    Some(Arc::new(crate::fused_agent::FusedAgent::new(
                        generator_arc.clone(),
                        tcs_for_fused,
                    )))
                } else {
                    warn!("Generator Arc not available for fused agent; skipping");
                    None
                }
            } else {
                None
    ...
    pub async fn set_topology_mode(&mut self, mode: TopologyMode) -> Result<()> {
        ...
        {
            let mut guard = self.config_arc.write().await;
...
fn tokenizer_path_from_env() -> Result<PathBuf> {
    ...
    // Fallback: try common locations (use environment variables if available)
    let workspace_models = env_value("WORKSPACE_MODELS_DIR")
        .unwrap_or_else(|| "/workspace/models".to_string());
    let fallback_paths = vec![
        format!("{}/Qwen2.5-7B-Instruct-AWQ/tokenizer.json", workspace_models),
        format!("{}/Qwen2-0.5B-Instruct/tokenizer.json", workspace_models),
