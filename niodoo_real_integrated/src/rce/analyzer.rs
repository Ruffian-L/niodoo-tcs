...
mod tests {
    ...
    fn test_beta_meta_basic() {
        let mut analyzer = RceAnalyzer::new(
            4,
            BetaMetaWeights { alpha_betti: 1.0, alpha_meta: 1.0, alpha_motif: 0.0, alpha_sheaf: 0.0 },
            0.5,
        );
        let pad = PadGhostState { pad: [0.1, 0.2, 0.3, 0.0, 0.0, 0.0, 0.0], mu: [0.0; 7], sigma: [0.1; 7], entropy: 2.0 };
        let topo = TopologicalSignature::new(
            vec![PersistentFeature { birth: 0.0, death: 1.0, dimension: 0 }],
            [1, 0, 0],
            0.1,
