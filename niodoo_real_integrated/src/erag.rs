    ...
    async fn collapse_with_grpc(
        ...
        match qdrant_client.search_points(search_request).await {
            ...
            Err(e) => {
                ...
                if err_msg.contains("OutputTooSmall") || err_msg.contains("collection is empty") {
                    warn!("Qdrant collection appears empty (OutputTooSmall), returning empty result");
                ...
                // Fallback to HTTP
                self.collapse_with_http(vector, limit).await
            }
        }
    }
