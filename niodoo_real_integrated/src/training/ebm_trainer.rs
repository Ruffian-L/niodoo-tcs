...
use candle_nn::{Adam, Optimizer, VarBuilder, VarMap};
use ndarray::Array1;
use rand::Rng;
...
impl EBMTrainer {
    ...
    pub fn new(
        ...
        // Note: Simplified optimizer creation - full implementation would use
        // candle's optimizer API properly
        let params = energy_net.parameters();
        let optimizer = Adam::new(params, learning_rate)?;
        
        info!(
            "Initialized EBMTrainer: input_dim={}, hidden_dim={}, langevin_steps={}",
            input_dim, hidden_dim, langevin_steps
        );
    ...
    fn langevin_sample(&self, initial_samples: &Tensor) -> Result<Tensor> {
        ...
        for step in 0..self.langevin_steps {
            ...
            samples = samples.sub(&noise.broadcast_mul(&Tensor::new(
                &[self.langevin_step_size],
                samples.device(),
            )?)?)?;
            
            if step % 10 == 0 {
                debug!("Langevin step {}: energy={:.6}", step, energy.mean_all()?.to_scalar::<f32>()?);
            }
        }
