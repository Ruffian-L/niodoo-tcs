//! Curator Ablation Study
//!
//! Tests the impact of disabling curator on response quality assessment.
...
/// Run curator ablation study
pub async fn run_curator_ablation(
    config: AblationConfig,
    test_prompts: Vec<String>,
    output_dir: PathBuf,
) -> anyhow::Result<AblationResult> {
    info!("Running curator ablation study with {} prompts", test_prompts.len());

    let mut cli_args = CliArgs::default();
    cli_args.no_curator = true;
    ...
    for prompt in test_prompts {
        ...
        match pipeline_guard.process_prompt(&prompt).await {
            Ok(cycle) => {
                ...
                // Note: curator_quality will be None when curator is disabled
                // This is expected and part of the ablation test
                if let Some(quality) = cycle.generation.curator_quality {
                    metrics.response_quality.push(quality);
                }
            }
            Err(e) => {
                tracing::warn!(error = %e, "Failed to process prompt in curator ablation");
            }
        }
    }
