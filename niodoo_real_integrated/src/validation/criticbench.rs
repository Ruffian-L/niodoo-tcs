...
impl CriticBenchRunner {
    ...
    pub fn load_tasks<P: AsRef<std::path::Path>>(path: P) -> Result<Vec<CriticBenchTask>, anyhow::Error> {
    ...
    fn evaluate_correction(&self, original: &str, correction: &str, task: &CriticBenchTask) -> f64 {
        ...
        let expected_matches = task.expected_corrections.iter()
        ...
        F: Fn(&str) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<String, anyhow::Error>> + Send>>,
    {
        ...
        for task in tasks {
            let result = self.run_task(task, &generate_fn, &critique_fn, &correct_fn).await?;
            results.push(result);
        }

        let avg_generation_score = results.iter().map(|r| r.generation_score).sum::<f64>() / results.len() as f64;
        let avg_critique_score = results.iter().map(|r| r.critique_score).sum::<f64>() / results.len() as f64;
        let avg_correction_score = results.iter().map(|r| r.correction_score).sum::<f64>() / results.len() as f64;
        let improvement_rate = results.iter().filter(|r| r.improvement_detected).count() as f64 / results.len() as f64;
