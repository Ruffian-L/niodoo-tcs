...
impl LoCoMoRunner {
    ...
    pub fn load_tests<P: AsRef<std::path::Path>>(path: P) -> Result<Vec<LoCoMoTest>, anyhow::Error> {
        ...
        F: Fn(&str) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<String, anyhow::Error>> + Send>>,
    {
        ...
        for question in &test.questions {
            ...
            *category_keyword_match_sum.entry(cat_key.clone()).or_insert(0) += result.keywords_matched;
            *category_keyword_total_sum.entry(cat_key.clone()).or_insert(0) += result.keywords_total;
        ...
        for (cat_key, count) in category_counts {
            ...
            let exact_match_rate = category_exact_match_sum.get(&cat_key).copied().unwrap_or(0) as f64 / count as f64;
