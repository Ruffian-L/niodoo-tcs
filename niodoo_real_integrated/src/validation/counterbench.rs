...
impl CounterBenchRunner {
    ...
    pub fn load_tasks<P: AsRef<std::path::Path>>(path: P) -> Result<Vec<CounterBenchTask>, anyhow::Error> {
        ...
        F: Fn(&str) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<String, anyhow::Error>> + Send>>,
    {
        ...
        let avg_keyword_match_rate = results.iter().map(|r| r.keyword_match_rate).sum::<f64>() / results.len() as f64;
