...
async fn run_soak_test(iterations: usize) -> Result<()> {
    ...
    if metrics_per_10.len() >= 2 {
        let first_half: Vec<_> = metrics_per_10
            .iter()
            .take(metrics_per_10.len() / 2)
            .collect();
        let second_half: Vec<_> = metrics_per_10
            .iter()
            .skip(metrics_per_10.len() / 2)
            .collect();

        let first_avg_latency: f64 =
            first_half.iter().map(|(_, l, _, _, _)| l).sum::<f64>() / first_half.len() as f64;
        let second_avg_latency: f64 =
            second_half.iter().map(|(_, l, _, _, _)| l).sum::<f64>() / second_half.len() as f64;

        let degradation = ((second_avg_latency - first_avg_latency) / first_avg_latency) * 100.0;

        if degradation > 50.0 {
            warn!(
                "⚠️  Performance degradation detected: {:.1}% latency increase",
                degradation
            );
        } else {
