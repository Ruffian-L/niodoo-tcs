...
fn create_test_batch(size: usize, input_dim: usize, output_dim: usize) -> Vec<(Vec<f32>, Vec<f32>)> {
    use rand::Rng;
...
fn test_weights_actually_update() -> Result<()> {
    ...
    println!("✅ Weight update test: diff = {:.9}, loss = {:.6}", total_diff, loss);
...
fn test_gradients_exist() -> Result<()> {
    ...
    println!("✅ Gradient test: loss={:.6}, weight_diff={:.9}", loss, total_diff);
