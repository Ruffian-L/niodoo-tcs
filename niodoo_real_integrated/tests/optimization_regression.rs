...
use niodoo_real_integrated::{
    ...
    erag::EragClient,
    gpu_fitness::GPUMemoryFitnessCalculator,
    memory_consolidation::MemoryConsolidationManager,
    pipeline::state::CuratorFeedbackController,
    torus::PadGhostState,
    weighted_episodic_mem::TemporalDecayConfig,
};
...
fn test_crdt_consolidation_idempotency() {
    ...
    // First merge
    let result1 = manager.crdt_merge_consolidation(
        memory_id,
        consolidation_level,
        fitness_score,
        timestamp,
    ...
    let result2 = manager.crdt_merge_consolidation(
        memory_id,
        consolidation_level,
        fitness_score,
        timestamp,
