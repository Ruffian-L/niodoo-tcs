...
mod temporal_tda_tests {
    ...
    impl TemporalTDADetector {
        ...
        fn detect_failure_chain(&mut self, events: Vec<FailureEvent>) -> Option<FailureChain> {
            ...
            for i in 0..events.len() - 1 {
                let dist = self.calculate_wasserstein_distance(
                    &events[i].snapshot,
                    &events[i + 1].snapshot,
                );
                distances.push(dist);
        ...
        fn detect_danger_signature(&self) -> Option<DangerSignature> {
            ...
            let network_degrading = recent[0].ghost_network_stability < recent[1].ghost_network_stability
        ...
        fn calculate_healing_convergence_time(
            ...
            for snapshot in self.snapshot_window.iter().rev() {
                if snapshot.timestamp > intervention_time
                    ...
                    && snapshot.beta_1 < 0.4 // Healthy β₁ range
                {
    ...
    fn test_wasserstein_distance_master_vs_panic() {
        ...
        assert!(distance > 0.5, "Distance {} should indicate significant drift", distance);
    }
    ...
    fn test_wasserstein_distance_stable_states() {
        ...
        assert!(distance < 0.15, "Distance {} should be small for stable states", distance);
    }
    ...
    fn test_progressive_ghost_amplification_stress() {
        ...
        assert!(final_snapshot.entropy > 5.0, "Entropy should indicate chaos");
    ...
    fn test_benchmark_50_percent_chain_reduction() {
        ...
        let reduction_percent = ((baseline_failures - with_tda_failures) as f32 / baseline_failures as f32) * 100.0;
