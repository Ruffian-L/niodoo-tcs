...
mod federated_tda_tests {
    ...
    impl NiodooInstance {
        ...
        fn experience_failure_chain(&mut self, events: Vec<FailureEvent>) {
            ...
            if let Some(chain) = self.detector.detect_failure_chain(events.clone()) {
                ...
                let barcode = FailureBarcode {
                    chain_id: format!("{}_{}", self.id.0, chain.events[0].timestamp.elapsed().as_millis()),
        ...
        fn learn_from_federation(&mut self) {
            ...
            for barcode in top_barcodes {
                ...
                if self.learned_from_federation.iter().any(|b| b.chain_id == barcode.chain_id) {
                    continue;
                }
        ...
        fn can_avoid_similar_failure(&self, failure_type: &FailureType, pattern: &ChainPatternType) -> bool {
            // Check if we've learned from a similar failure through federation
            self.learned_from_federation.iter().any(|barcode| {
                barcode.pattern_type == *pattern
            })
        }
    ...
    fn test_federated_failure_barcode_submission() {
        ...
        assert_eq!(barcodes[0].instance_id, InstanceId("instance_a".to_string()));
    ...
    fn test_federated_learning_propagation() {
        ...
        for i in 0..9 {
            ...
            events_a.push(FailureEvent {
                failure_type: if i == 7 { FailureType::ConnectionDropout } else { FailureType::RateLimit },
                timestamp: snapshot.timestamp,
                snapshot,
            });
        }
    ...
    fn test_collective_void_avoidance() {
        ...
        let avoided_count = instances
            .iter()
            .skip(1)
            .filter(|i| i.can_avoid_similar_failure(&FailureType::Overload, &ChainPatternType::OverloadSpiral))
            .count();

        assert_eq!(avoided_count, 4, "All 4 instances should learn to avoid the pattern");
    ...
    fn test_swarm_resource_balancing_under_stress() {
        ...
        assert_eq!(learned_instances, 7, "All healthy instances should learn from stressed peers");
    ...
    fn test_adversarial_resilience_uncooperative_instance() {
        ...
        assert_eq!(functioning_instances, 4, "Cooperative swarm should still function");
    ...
    fn test_global_entropy_stability_under_swarm_stress() {
        ...
        assert_eq!(learned_count, 6, "Healthy instances preemptively learn from stressed peers");
        ...
        assert!(avg_severity_per_instance > 50.0, "Individual failures were significant");
    }
    ...
    fn test_federated_qlora_batch_construction() {
        ...
        for instance_id in 0..5 {
            let mut instance = NiodooInstance::new(format!("instance_{}", instance_id), coordinator.clone());
            let base_time = Instant::now();
