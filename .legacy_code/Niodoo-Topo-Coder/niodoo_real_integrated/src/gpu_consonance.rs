...
impl GpuConsonanceCalculator {
    ...
    pub async fn batch_pad_variance(&self, pad_states: &[[f64; 7]]) -> Result<Vec<f64>> {
        ...
        task::spawn_blocking(move || {
            ...
            let means = pad_tensor.sum_keepdim(1)?.broadcast_div(&Tensor::new(&[3.0f32], &device)?)?;
            
            // Compute variances: [batch, 1]
            let centered = pad_tensor.sub(&means.expand((batch_size, 3))?)?;
            let variances = centered.sqr()?.sum_keepdim(1)?.broadcast_div(&Tensor::new(&[3.0f32], &device)?)?;
        ...
        }).await.map_err(|e| anyhow::anyhow!("GPU task panicked: {}", e))?
    }
