...
/// Zero-copy ndarray to NumPy conversion helper
///
/// This function demonstrates the zero-copy transfer capability.
/// The numpy::PyArray2::from_array_bound() function uses zero-copy
/// when the memory layout is compatible.
pub fn matrix_to_numpy_zero_copy<'py>(
    py: Python<'py>,
    matrix: &Array2<f64>,
) -> Result<Bound<'py, numpy::PyArray2<f64>>, TdaError> {
    // numpy::PyArray2::from_array_bound performs zero-copy conversion
    // when the ndarray is C-contiguous and has compatible dtype
    Ok(numpy::PyArray2::from_array_bound(py, matrix))
}
