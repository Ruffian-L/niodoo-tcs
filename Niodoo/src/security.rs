...
const DEFAULT_PROMPT_MAX_CHARS: usize = 4096;
...
pub struct SecurityConfig {
    ...
    #[serde(default = "default_banned_patterns")]
...
impl SecurityConfig {
    pub fn from_file(path: &str) -> Result<Self> {
        let content = std::fs::read_to_string(path)
            .with_context(|| format!("failed to read security config from {}", path))?;
        let mut config: SecurityConfig =
            toml::from_str(&content).context("failed to parse security config")?;
        config.finalize();
        Ok(config)
    }
    ...
    fn finalize(&mut self) {
        if self.prompt_max_chars == 0 {
            self.prompt_max_chars = DEFAULT_PROMPT_MAX_CHARS;
        }
        if self.banned_patterns.is_empty() {
            self.banned_patterns = default_banned_patterns();
        }
        if self.audit_log_path.trim().is_empty() {
            self.audit_log_path = default_audit_log_path();
        }
        if self.rate_limit_window_secs == 0 {
            self.rate_limit_window_secs = default_rate_limit_window_secs();
        }
        if self.rate_limit_max_requests == 0 {
            self.rate_limit_max_requests = default_rate_limit_max_requests();
        }
    }
}
...
fn default_banned_patterns() -> Vec<String> {
    vec![
...
impl PromptSecurityManager {
    ...
    pub fn audit_config_snapshot<T>(&self, config: &T)
    where
        T: Serialize,
    {
...
impl RateLimiter {
    fn new(window_secs: u64, max_requests: u32) -> Self {
        let window = if window_secs == 0 {
            warn!("rate limit window_secs is 0; defaulting to 1 second. this may not be intended.");
...
impl AuditLogger {
    ...
    fn log_config_snapshot<T>(&self, config: &T) -> Result<()>
    where
        T: Serialize,
    {
        let json =
            to_json_string(config).context("failed to serialise security config for audit")?;
        let digest = blake3::hash(json.as_bytes());
        let line = format!(
            "{} event=config_snapshot hash={} char_count={}",
            Utc::now().to_rfc3339(),
            hex::encode(digest.as_bytes()),
            json.chars().count()
        );
