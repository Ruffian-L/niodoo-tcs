...
"#,
    ...
    pub fn query_mobius_memories(
        ...
        let similarity_score = if query_embedding.len() > 0 { 0.7 } else { 0.0 };
        let _relevance_score = emotional_context * similarity_score;
...
mod tests {
    ...
    #[test]
    #[ignore = "Requires Python modules and embedding models - set NIODOO_ENABLE_REAL_MEMORY_BRIDGE=1 to run"]
    fn test_store_and_retrieve() {
        if std::env::var("NIODOO_ENABLE_REAL_MEMORY_BRIDGE").is_err() {
            return;
        }
