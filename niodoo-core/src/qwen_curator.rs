...
use candle_nn::{
    ...
    loss, AdamW, Optimizer, ParamsAdamW, VarBuilder, VarMap,
};
...
impl QloraCurator {
    ...
    fn compute_loss(&mut self, input_ids: &[u32], label_ids: &[u32]) -> Result<f64> {
        ...
        let model = self
            .model
            .as_mut()
            .ok_or_else(|| anyhow!("Model not loaded - call load_model() before training"))?;
...
mod tests {
    ...
    #[tokio::test]
    #[ignore = "Requires Python modules and Qwen models - set NIODOO_ENABLE_QWEN_CURATOR=1 to run"]
    async fn test_curator_initialization() {
        if std::env::var("NIODOO_ENABLE_QWEN_CURATOR").is_err() {
            return;
        }
