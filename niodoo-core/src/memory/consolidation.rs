...
impl MemoryConsolidationEngine {
    ...
    fn get_similarity_threshold(&self) -> f32 {
        use constants_core::consciousness::{
            memory_similarity_threshold, MEMORY_SIMILARITY_SCALING_DIVISOR,
        };
        ...
        let memory_count = self
            ...
            .map(|m| m.len())
            .unwrap_or(0);
        let base_threshold = memory_similarity_threshold();
        let adaptive_factor = (memory_count as f32 / MEMORY_SIMILARITY_SCALING_DIVISOR).min(0.3); // Max adjustment of 0.3
