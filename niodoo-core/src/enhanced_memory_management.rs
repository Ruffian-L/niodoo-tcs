...
pub struct SimpleMemoryManager {
    ...
    deduplication_engine: Arc<Mutex<DeduplicationEngine>>,
    /// Garbage collector
    garbage_collector: Arc<Mutex<GarbageCollector>>,
    /// Memory statistics tracker
    stats_tracker: Arc<Mutex<MemoryStatsTracker>>,
    /// Configuration
...
impl SimpleMemoryManager {
    ...
    pub async fn new(config: MemoryManagerConfig) -> Result<Self> {
        ...
        let compression_engine = Arc::new(Mutex::new(CompressionEngine::new(
            config.compression_config.clone(),
        )?));
        let deduplication_engine = Arc::new(Mutex::new(DeduplicationEngine::new(
            config.deduplication_config.clone(),
        )?));
        let garbage_collector = Arc::new(Mutex::new(
            GarbageCollector::new(config.gc_config.clone()).await?,
        ));
        let stats_tracker = Arc::new(Mutex::new(MemoryStatsTracker::new()));

        // Start background garbage collection
        garbage_collector.lock().await.start_gc_cycle().await;
    ...
    pub async fn allocate_optimized(&self, size: usize) -> Result<OptimizedMemory> {
        ...
        self.stats_tracker.lock().await.record_allocation(size);
        ...
        if size > self.config.compression_threshold {
            if let Ok(compressed) = self
                ...
                .await
            {
                self.stats_tracker.lock().await.record_compression();
    ...
    pub async fn store_string_optimized(&self, content: String) -> Result<StringHandle> {
        ...
        if let Some(deduped) = self
            .deduplication_engine
            .lock()
            .await
            .find_duplicate(&content)
            .await?
        {
            self.stats_tracker.lock().await.record_deduplication_hit();
        ...
        let handle = self
            ...
            .lock()
            .await
            .store_unique(content, processed_content)
            .await?;
    ...
    pub async fn optimize_memory_usage(&self) -> Result<MemoryOptimizationResult> {
        ...
        let dedup_result = self
            ...
            .await?;
        total_deduped += dedup_result.duplicates_found;
        ...
        info!(
            "✅ Simplified memory optimization complete: freed={}, compressed={}, deduped={}",
    ...
    pub async fn get_memory_stats(&self) -> Result<MemoryStats> {
        let compression_stats = self.compression_engine.lock().await.get_stats().await?;
        let deduplication_stats = self.deduplication_engine.lock().await.get_stats().await?;
        let gc_stats = self.garbage_collector.lock().await.get_stats().await?;
        let stats_tracker = self.stats_tracker.lock().await;

        Ok(MemoryStats {
            total_allocations: stats_tracker.total_allocations(),
            total_bytes_allocated: stats_tracker.total_bytes_allocated(),
    ...
    async fn calculate_memory_pressure(&self) -> Result<f64> {
        // Access stats_tracker directly to avoid recursion
        let stats_tracker = self.stats_tracker.lock().await;
        let total_bytes = stats_tracker.total_bytes_allocated();

        // Simple pressure calculation based on allocation patterns
        let pressure = (total_bytes as f64 / self.config.max_memory_bytes as f64).min(1.0);
    ...
    pub async fn force_gc(&self) -> Result<GcResult> {
        self.garbage_collector.lock().await.collect_garbage().await
    }
    ...
    pub async fn update_config(&mut self, config: MemoryManagerConfig) -> Result<()> {
        ...
        self.compression_engine
            .lock()
            .await
            .update_config(config.compression_config)
            .await?;
        self.deduplication_engine
            .lock()
            .await
            .update_config(config.deduplication_config)
            .await?;
        self.garbage_collector
            .lock()
            .await
            .update_config(config.gc_config)
            .await?;
    ...
    pub async fn shutdown(&self) -> Result<()> {
        ...
        self.garbage_collector.lock().await.stop_gc_cycle().await;
...
impl CompressionEngine {
    ...
    pub async fn compress_memory(&self, memory: &OptimizedMemory) -> Result<OptimizedMemory> {
        ...
        let mut compressed_memory =
            OptimizedMemory::from_data(compressed_data, memory.purpose.clone());
...
impl DeduplicationEngine {
    ...
    pub async fn analyze_and_optimize(&self) -> Result<DedupOptimizationResult> {
        ...
        for (_content, &count) in dup_counts.iter() {
            if count > 1 {
                duplicates_found += 1;
        ...
        if dedup_ratio > self.config.max_dedup_ratio {
            warn!(
                "High deduplication ratio detected: {:.2}%",
                dedup_ratio * 100.0
            );
        }
...
impl GarbageCollector {
    ...
    pub async fn start_gc_cycle(&self) {
        ...
        let gc_handle = tokio::spawn(async move {
            ...
            loop {
                ...
                if pressure > max_pressure {
                    info!(
                        "🔄 Memory pressure high ({:.2}%), running garbage collection",
                        pressure
                    );
                    ...
                    if gc_result.bytes_freed > 0 {
                        info!(
                            "✅ Garbage collection freed {} bytes",
        ...
        *self.gc_handle.lock().await = Some(gc_handle);
