...
impl QwenConfig {
    /// Qwen3-Embedding 4B configuration (zhiqing/Qwen3-Embedding-4B-ONNX)
    /// Source: https://huggingface.co/zhiqing/Qwen3-Embedding-4B-ONNX/blob/main/config.json
    pub fn qwen3_embedding_4b() -> Self {
        Self {
            // Match ONNX graph: 24 layers, 2 KV heads, head_dim=64
            num_layers: 24,
            num_heads: 32,
            num_kv_heads: Some(2),
            head_dim: 64,
    ...
    pub fn qwen25_coder_05b() -> Self {
        Self {
            ...
            num_kv_heads: Some(2), // KV heads for GQA (model expects 2 KV heads)
    ...
    pub fn validate(&self) -> anyhow::Result<()> {
        ...
        if let Some(kv_heads) = self.num_kv_heads {
            if kv_heads == 0 {
                return Err(anyhow::anyhow!("num_kv_heads must be > 0 if specified"));
            }
