...
mod inference_backend {
    ...
    #[cfg(feature = "onnx")]
    pub use self::mock::ModelBackend;
    // Note: onnx module is commented out and needs ort 2.0 API fixes
    // When enabled, uncomment and fix API calls

    // Temporarily disabled - legacy onnx module has ort 2.0 API issues
    // #[cfg(all(feature = "onnx", feature = "legacy-onnx"))]
    // mod onnx {
    ...
    //                 // ort 2.0: SessionBuilder manages environment internally
    //                 Ok(Self {
    //                     name: name.to_string(),
    ...
    //                 // ort 2.0: SessionBuilder manages environment internally, no need for ort_init()
    ...
    //                         .into_dyn();
    //
    //                 // ort 2.0: Convert to Vec and create Tensor, then Value
    //                 let mem = MemoryInfo::from_location(&Location::Cpu)
    //                     .context("failed to create memory info")?;
    //
    //                 let input_ids_shape: Vec<i64> = input_ids_tensor.shape().iter().map(|&d| d as i64).collect();
    //                 let input_ids_data: Vec<i64> = input_ids_tensor.iter().copied().collect();
    //                 let input_ids_ort_tensor = Tensor::from_data(&input_ids_data, &input_ids_shape, DataType::Int64, &mem)
    //                     .context("failed to create input_ids tensor")?;
    //                 let input_ids_value = Value::from_tensor(input_ids_ort_tensor)
    //                     .context("failed to create input_ids ORT value")?;
    //
    //                 let attention_mask_shape: Vec<i64> = attention_mask_tensor.shape().iter().map(|&d| d as i64).collect();
    //                 let attention_mask_data: Vec<i64> = attention_mask_tensor.iter().copied().collect();
    //                 let attention_mask_ort_tensor = Tensor::from_data(&attention_mask_data, &attention_mask_shape, DataType::Int64, &mem)
    //                     .context("failed to create attention_mask tensor")?;
    //                 let attention_mask_value = Value::from_tensor(attention_mask_ort_tensor)
    //                     .context("failed to create attention_mask ORT value")?;
    //
    //                 // ort 2.0: Session::run() takes Vec<Value> directly
    //                 let inputs = vec![input_ids_value, attention_mask_value];
    //                 let outputs = session
    //                     .run(&inputs, &[])
    //                     .context("failed to execute ONNX session")?;
    //
    //                 // In ort 2.0, SessionOutputs iterator returns (&str, Value) tuples
    //                 let outputs_vec: Vec<(&str, Value)> = outputs.into_iter().collect();
    //                 let (_, first_output) = outputs_vec
    //                     .first()
    ...
    //                         .into_dyn();
    //
    //                 // ort 2.0: Value::from_array() needs (shape, Vec<data>) tuple format
    //                 let input_ids_shape = [1i64, input_ids.len() as i64];
    //                 let input_ids_value = Value::from_array((input_ids_shape.as_slice(), input_ids.clone()))
    //                     .context("failed to create input_ids ORT value")?;
    //
    //                 let attention_mask_shape = [1i64, attention_mask.len() as i64];
    //                 let attention_mask_value = Value::from_array((attention_mask_shape.as_slice(), attention_mask.clone()))
    //                     .context("failed to create attention_mask ORT value")?;
    //
    //                 // ort 2.0: run() takes &[Value] and optional output names
    //                 let inputs = vec![input_ids_value, attention_mask_value];
    //                 let outputs = session
    //                     .run(&inputs, &[])
    //                     .context("failed to execute ONNX session")?;
    //
    //                 // ort 2.0: run() returns Vec<Value> directly
    ...
    //                             Ok(tensor) => {
    //                                 let view = tensor.view();
    //                                 view.iter().map(|&x| f16::to_f32(x)).collect()
    //                             }
    //                             Err(e) => {
    //                                 return Err(anyhow!(
    ...
    #[cfg(any(not(feature = "onnx"), feature = "onnx"))]
    pub mod mock {
        use anyhow::Result;
        use std::sync::{Arc, Mutex};
...
pub use inference_backend::ModelBackend;
pub use ModelBackend as InferenceModelBackend;
