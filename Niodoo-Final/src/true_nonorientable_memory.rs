...
impl TopologicalCoordinate {
    ...
    pub fn to_floats(&self) -> (f32, f32, i32) {
        (self.u_millis as f32 / 1000.0, self.v_millis as f32 / 1000.0, self.k)
    }
}
...
impl TrueNonOrientableMemory {
    ...
    pub fn new() -> Self {
        ...
        for (i, layer_type) in layer_types.into_iter().enumerate() {
    ...
    pub fn traverse_non_orientable(&mut self, query: &str, max_depth: usize) -> Vec<MemoryFragment> {
    ...
    fn traverse_topologically(
        ...
        for mut fragment in layer_results {
            ...
            let orientation_multiplier = self.calculate_topological_relevance(&fragment, &current_coord);
    ...
    fn get_current_coordinate(&self) -> TopologicalCoordinate {
        ...
        let layer_angle = (self.traversal_state.current_layer as f32 / 6.0) * 2.0 * std::f32::consts::PI;
    ...
    fn search_topological_region(&self, query: &str, coordinate: &TopologicalCoordinate) -> Vec<MemoryFragment> {
        let mut results = Vec::new();
        ...
        for fragment in &self.fragments {
            ...
            if distance < 0.5 { // Threshold for topological proximity
    ...
    fn calculate_topological_distance(&self, coord1: &TopologicalCoordinate, coord2: &TopologicalCoordinate) -> f32 {
    ...
    fn calculate_topological_relevance(&self, fragment: &MemoryFragment, current_coord: &TopologicalCoordinate) -> f32 {
        ...
        let topological_distance = self.calculate_topological_distance(current_coord, fragment_coord);
    ...
    fn generate_next_coordinates(&self, current_coord: &TopologicalCoordinate) -> Vec<TopologicalCoordinate> {
    ...
    pub fn add_memory(&mut self, content: String, layer_type: LayerType) -> Result<(), String> {
        // Find the layer
        let layer_index = self.layers.iter().position(|l| l.layer_type == layer_type)
    ...
    pub fn get_stats(&self) -> MemoryStats {
        ...
        let avg_relevance = if total_fragments > 0 {
            self.layers.iter().flat_map(|l| l.fragments.iter().map(|f| f.relevance)).sum::<f32>() / total_fragments as f32
        } else {
            0.0
        };
...
impl fmt::Display for TrueNonOrientableMemory {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(f, "  Current state: Layer {}, Orientation: {:?}", 
                 self.traversal_state.current_layer,
                 self.traversal_state.orientation)?;
        ...
        for (i, layer) in self.layers.iter().enumerate() {
            writeln!(f, "    {}: {} fragments ({:?})",
                     i, layer.fragments.len(), layer.layer_type)?;
        }
...
pub fn demonstrate_standalone_topology() {
    ...
    tracing::info!("  • Coord 1 integer: u={}, v={}, k={}", coord1.u_millis, coord1.v_millis, coord1.k);
    ...
    tracing::info!("  • Twist boundary distance: {:.3} (should be small)", twist_distance);
    tracing::info!("--- Topology Demo Separator ---");
...
fn calculate_simple_distance(coord1: &TopologicalCoordinate, coord2: &TopologicalCoordinate) -> f32 {
    let (u1, v1, k1) = coord1.to_floats();
    let (u2, v2, k2) = coord2.to_floats();
...
pub fn demonstrate_real_topological_traversal() {
    ...
    tracing::info!("  3. **Distance Metric**: Geodesic distance accounting for non-orientable property");
    tracing::info!("  4. **Orientation Effects**: Flipping affects coordinate generation and relevance");
...
mod tests {
    ...
    fn test_non_orientable_creation() {
        ...
        assert!(matches!(memory.traversal_state.orientation, Orientation::Normal));
    }
    ...
    fn test_memory_addition() {
        ...
        let result = memory.add_memory(
            "Test memory fragment".to_string(),
            LayerType::Semantic
        );
    ...
    fn test_non_orientable_traversal() {
        ...
        memory.add_memory("consciousness test".to_string(), LayerType::Semantic).unwrap();
        memory.add_memory("memory traversal".to_string(), LayerType::Episodic).unwrap();
        ...
        for fragment in &results {
            assert!(matches!(fragment.orientation, Orientation::Normal | Orientation::Flipped));
        }
