...
/// Z-score normalisation helper for f32 slices.
///
/// Returns `false` if the slice is empty or the variance collapses to zero.
pub fn standardize_vector(values: &mut [f32]) -> bool {
    if values.is_empty() {
        return false;
    }
    ...
    if !stdev.is_finite() || stdev <= f32::EPSILON {
        return false;
    }
...
/// Apply `standardize_vector` to each row. Empty rows are ignored.
pub fn standardize_rows(rows: &mut [Vec<f32>]) {
    for row in rows.iter_mut() {
        standardize_vector(row);
    }
}
...
impl TDAFeatureExtractor {
    ...
    pub fn extract_features(
        ...
        if !lifetimes.is_empty() {
            ...
            let max_lifetime = lifetimes.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
        ...
        for &lifetime in lifetimes {
            if lifetime > 0.0 {
                let bin_idx = ((lifetime / max_lifetime) * (self.persistence_resolution - 1) as f64)
                    .min((self.persistence_resolution - 1) as f64)
                    as usize;
...
mod tests {
    ...
    fn test_feature_extraction() -> Result<()> {
        ...
        let persistence_diagram =
            Array2::from_shape_vec((3, 2), vec![0.0, 1.0, 0.5, 2.0, 1.0, 1.5])?;
