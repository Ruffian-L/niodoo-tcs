...
impl QwenStatefulEmbedder {
    pub fn new(model_path: impl AsRef<Path>, expected_dim: usize) -> Result<Self> {
        ...
        let handle = std::thread::spawn(move || {
            let result = QwenEmbedder::new(&model_path_str).with_context(|| {
                format!("failed to initialise Qwen embedder from {}", model_path_str)
            });
            tx.send(result).ok();
        });
        ...
        let embedder = match rx.recv_timeout(std::time::Duration::from_secs(timeout_secs)) {
            ...
            Err(_) => {
                ...
                return Err(anyhow!(
                    "Qwen embedder initialization timed out after {} seconds",
                    timeout_secs
                ));
            }
        };

        let resolved_dim = embedder.embed_dim();
        if resolved_dim != expected_dim {
            warn!(
                expected = expected_dim,
                resolved = resolved_dim,
                "Embedding dimension override detected; aligning runtime vector dim"
            );
        }

        Ok(Self {
            inner: Arc::new(Mutex::new(embedder)),
            vector_dim: Arc::new(AtomicUsize::new(resolved_dim)),
        })
    }
    ...
    fn new_mock(_expected_dim: usize) -> Result<Self> {
        ...
        Err(anyhow!(
            "Mock embedder requires MOCK_MODE env var set in tcs_ml - provide a valid model path"
        ))
    }
}
...
impl Embedder for QwenStatefulEmbedder {
    ...
    async fn embed(&self, prompt: &str) -> Result<Vec<f32>> {
        ...
        let vector_dim = self.vector_dim.clone();
        ...
        if let Err(ref err) = embedding {
            // Log full QwenError including any underlying OrtError before mapping to anyhow
            error!(
                target: "niodoo_real_integrated::embedding",
                error = ?err,
                "QwenStatefulEmbedder::embed failed"
            );
        }

        let mut embedding = embedding.map_err(|e: QwenError| anyhow!(e))?;
        let actual_dim = embedding.len();
        let mut target_dim = vector_dim.load(Ordering::Relaxed);

        if actual_dim != target_dim {
            info!(
                actual = actual_dim,
                previous = target_dim,
                "Embedding length changed; updating runtime vector dimension"
            );
            vector_dim.store(actual_dim, Ordering::Relaxed);
...
impl QwenStatefulEmbedder {
    ...
    pub fn vector_dim(&self) -> usize {
        self.vector_dim.load(Ordering::Relaxed)
    }
}
