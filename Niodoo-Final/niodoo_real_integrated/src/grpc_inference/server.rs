...
use proto::{
    ...
    HealthCheckResponse, InferenceMetadata, InferenceOptions, InferenceRequest, InferenceResponse,
    LoadModelRequest, LoadModelResponse, ModelInfo, Tensor, TensorDataType, TensorShape,
};
        ...
        for input in &session.inputs {
            if let Some(shape) = &input.input_type.shape {
                let dims: Vec<i64> = shape
                    ...
                    .collect();
                input_shapes.insert(input.name.clone(), TensorShape { dims });
            }
        }
        ...
        for output in &session.outputs {
            if let Some(shape) = &output.output_type.shape {
                let dims: Vec<i64> = shape
                    ...
                    .map(|d| d.dim_value().unwrap_or(-1) as i64)
                    .collect();
                output_shapes.insert(output.name.clone(), TensorShape { dims });
            }
        }
...
impl OnnxInferenceService for OnnxInferenceServer {
        ...
        #[cfg(feature = "onnx")]
        {
            ...
            for (name, tensor) in &req.inputs {
                let ort_value = self
                    ...
                    .map_err(|e| {
                        Status::internal(format!("Failed to convert tensor {}: {}", name, e))
                    })?;
    ...
    type StreamInferStream =
        tokio_stream::wrappers::ReceiverStream<Result<InferenceResponse, Status>>;
            ...
            x if x == TensorDataType::TensorDataTypeFloat32 as i32 => {
                let data: Vec<f32> = if !tensor.float_data.is_empty() {
                    ...
                    if tensor.data.len() % 4 != 0 {
                        return Err(anyhow!(
                            "Tensor data length ({}) is not a multiple of 4 bytes (f32 size)",
                            tensor.data.len()
                        ));
                    }
                    tensor
            ...
            x if x == TensorDataType::TensorDataTypeInt64 as i32 => {
                let data: Vec<i64> = if !tensor.int64_data.is_empty() {
                    ...
                    if tensor.data.len() % 8 != 0 {
                        return Err(anyhow!(
                            "Tensor data length ({}) is not a multiple of 8 bytes (i64 size)",
                            tensor.data.len()
                        ));
                    }
                    tensor
...
pub async fn start_server(addr: std::net::SocketAddr, gpu_device_id: i32) -> Result<()> {
    let server =
        OnnxInferenceServer::new(gpu_device_id).context("Failed to create inference server")?;
