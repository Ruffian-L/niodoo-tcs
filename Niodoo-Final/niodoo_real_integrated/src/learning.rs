...
const DEFAULT_REFLECTION_BUFFER_MIN: usize = 4;
...
use crate::lora_trainer::{LoRAConfig, LoRATrainer, LoRAUpdateStats};
...
pub struct LearningOutcome {
    ...
    pub lora_update: Option<LoRAUpdateStats>,
}
...
pub struct LearningLoop {
    ...
    reflection_buffer: Vec<ReflectionSample>,
    reflection_batch_min: usize,
    lora_epochs: usize,
    #[allow(dead_code)]
...
impl LearningLoop {
    pub fn new(
        ...
        let reflection_batch_min = std::env::var("REFLECTION_BUFFER_MIN")
            .ok()
            .and_then(|value| value.parse::<usize>().ok())
            .unwrap_or(DEFAULT_REFLECTION_BUFFER_MIN);
        ...
        let training_client = {
            ...
            if guard.training_service_enabled {
                let client = crate::training_service::TrainingServiceClient::new(
                    &guard.training_service_url,
                );
                Some(Arc::new(client))
            } else {
                None
                ...
                match result {
                    Ok(Ok(loss)) => {
                        info!(
                            ...
                            epochs, loss, "Async LoRA training completed"
                        );
                    }
                    Ok(Err(e)) => {
    ...
    fn queue_training_batch(
        ...
        #[cfg(feature = "svc")]
        {
            ...
            if let Some(client) = &self.training_client {
                ...
                tokio::spawn(async move {
                    match client_clone
                        .submit_training_job(samples_clone, epochs, learning_rate)
                        .await
                    {
                        Ok(job_id) => {
    ...
    pub async fn update(
            ...
            if !skip_qlora && self.average_reward() < qlora_low_reward_threshold {
                if run_qlora_async {
                    ...
                    tokio::spawn(async move {
                        ...
                        let _low_tuples = match erag_clone
                            .query_low_reward_tuples(qlora_low_reward_threshold, qlora_sample_count)
                            .await
                        {
                            Ok(tuples) => tuples,
                            Err(e) => {
        ...
        let lora_update = self.lora_trainer.read().last_update_stats();
    ...
    fn flush_curated_if_ready(&mut self, min_count: usize) {
        ...
        let training_samples: Vec<(Vec<f32>, Vec<f32>)> = self
            ...
            .map(|sample| {
                ...
                let mut target = sample.output.bytes().map(|b| b as f32).collect::<Vec<_>>();
    ...
    pub async fn apply_curator_learned(
        ...
        self.curated_buffer.push(CuratedSample {
            ...
            curator_quality: reward, // Use reward as curator quality proxy
        });
    ...
    pub fn enqueue_topology_reflection(
        &mut self,
        summary: &str,
        topology: &TopologicalSignature,
        reflection: &TopoReflection,
    ) {
        self.reflection_buffer.push(ReflectionSample {
        ...
        if self.reflection_buffer.len() < self.reflection_batch_min {
            return;
        }
        ...
        let samples: Vec<(Vec<f32>, Vec<f32>)> = self
            .reflection_buffer
            .iter()
            .map(|sample| {
                let mut features = vec![
                    sample.betti[0] as f32,
                    sample.betti[1] as f32,
                    sample.betti[2] as f32,
                    sample.spectral_gap as f32,
                    sample.persistence_entropy as f32,
                    sample.thinking_depth as f32,
                    sample.pivot_score as f32,
                ];
                while features.len() < embedding_dim {
                    features.push(0.0);
                }
                ...
                let mut target = format!("Slow down and structure the proof. {}", sample.summary)
                    .bytes()
                    .map(|byte| byte as f32)
                    .collect::<Vec<_>>();
                if target.len() < embedding_dim {
        ...
        self.queue_training_batch(samples, self.lora_epochs, 1e-3_f32);
        info!(
            count = self.reflection_buffer.len(),
            "Reflection-informed LoRA batch queued"
        );
        self.reflection_buffer.clear();
    }

    /// Add failure correction sample for contrastive QLoRA training
    pub async fn add_failure_correction(
        &mut self,
        prompt: &str,
        _bad_response: &str,
        ...
        // Use existing CuratedSample struct format (input, output, reward, topology metrics)
        let failure_sample = CuratedSample {
            input: prompt.to_string(),
            output: corrected_response.to_string(),
            reward: 1.5,          // Higher reward for learned corrections
            knot_complexity: 0.0, // Placeholder - would be computed from topology
            spectral_gap: 0.0,
            curator_quality: 1.0, // High quality since Gemini corrected it
        };

        self.curated_buffer.push(failure_sample);
        info!(
            "Added failure correction to curated buffer (size: {})",
            self.curated_buffer.len()
        );

        // Check if we should trigger QLoRA training
        let trigger_threshold = {
            let guard = self.config.read();
            guard.learning_qlora_sample_count
        };

        if self.curated_buffer.len() >= trigger_threshold {
            info!("🎯 Triggering QLoRA training (including failure corrections)");

            // Convert curated samples to training format using REAL embeddings
            let training_samples: Vec<(Vec<f32>, Vec<f32>)> = self
                .curated_buffer
                .iter()
                .map(|sample| {
            ...
            // Queue for training
            self.queue_training_batch(training_samples, self.lora_epochs, 1e-3);

            // Clear the buffer after queuing
            self.curated_buffer.clear();
            info!("Curated buffer cleared after QLoRA training trigger");
        }
    ...
    fn maybe_run_executor_distillation(&mut self) {
        ...
        if clusters.is_empty() {
            ...
            self.executor_memory.clear();
            self.executor_distill_threshold = (self.executor_distill_threshold + 8).min({
                let guard = self.config.read();
        ...
        for (cluster, score) in cluster_summaries.into_iter().take(3) {
            ...
            self.curated_buffer.push(CuratedSample {
                ...
                spectral_gap: 0.55,
                curator_quality: score as f64, // Use score as curator quality proxy
            });
            distilled_samples += 1;
        }
