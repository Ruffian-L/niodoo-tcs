...
use crate::behavior_trajectory::{BehaviorTrajectory, PointCloud};
...
impl TopologyMemoryAnalyzer {
    ...
    pub fn calculate_modularity(
        ...
        for edge_idx in graph.edge_indices() {
            let (source, target) = graph
                ...
                .ok_or_else(|| anyhow::anyhow!("Invalid edge index: {:?}", edge_idx))?;
    ...
    pub fn analyze_topology_with_beta1(
        ...
        let modularity = self
            .calculate_modularity(&graph, &communities)
            .unwrap_or(0.0);
    ...
    pub fn compute_h1_persistence(&self, point_cloud: &PointCloud) -> Result<Vec<(f64, f64)>> {
        ...
        for step in 0..num_filtration_steps {
            let epsilon = min_distance
                + (max_distance - min_distance) * (step as f64)
                    / (num_filtration_steps as f64 - 1.0);
    ...
    /// Uses simplified persistent homology computation via Vietoris-Rips filtration
    pub fn compute_h2_persistence(&self, point_cloud: &PointCloud) -> Result<Vec<(f64, f64)>> {
