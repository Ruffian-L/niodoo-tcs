...
pub fn prime_environment() {
    ...
    // Also check parent directory for niodoo_real_integrated.env
    if let Ok(current) = std::env::current_dir() {
    ...
    let env_files = [
        ".env.production",
        ".env",
        "niodoo_real_integrated.env",
        "../niodoo_real_integrated.env",
    ]; // ADDED: Unified environment + parent dir
    ...
    for root in roots {
        for file in env_files {
            ...
            let dedupe_key = match path.canonicalize() {
                Ok(canonical) => canonical,
                Err(error) => {
                    warn!(path = %path.display(), ?error, "failed to canonicalize env path; using relative path for dedupe");
                    path.clone()
                }
            };
...
pub fn init() {
    ...
    let qdrant_dim: usize = env_with_fallback(&["QDRANT_VECTOR_DIM", "QDRANT_VECTOR_SIZE"])
        ...
        .unwrap_or(2_560); // Aligned with Qwen3-Embedding-4B output dimensionality
    ...
    if ollama_url != "http://127.0.0.1:11434" {
        warn!(
            ...
            "OLLAMA_URL not default—ensure 'ollama serve && ollama pull llama3.1:8b'"
        );
    }
}
...
fn default_topocot_force_fallback() -> bool {
    // Keep existing default behaviour: fallback enabled
    true
}
...
fn default_rce_enabled() -> bool {
    true
}
fn default_rce_shadow_mode() -> bool {
    true
}
fn default_rce_actions_enabled() -> bool {
    false
}
fn default_rce_window_seconds() -> u64 {
    10
}
fn default_rce_stride_seconds() -> u64 {
    2
}
fn default_rce_beta_meta_weights() -> RceBetaMetaWeights {
    RceBetaMetaWeights::default()
}
fn default_rce_breakthrough_threshold() -> f64 {
    0.5
}
fn default_rce_erag_lambda() -> f64 {
    0.0
}
fn default_rce_archive_backend() -> String {
    "Qdrant".to_string()
}
...
/// Topology configuration applied to the TCS analyzer.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TcsRuntimeConfig {
    #[serde(default = "default_tcs_takens_dimension")]
    pub takens_dimension: usize,
    #[serde(default = "default_tcs_takens_delay")]
    pub takens_delay: usize,
    #[serde(default = "default_tcs_takens_history_len")]
    pub takens_history_len: usize,
    #[serde(default = "default_tcs_emotional_signal_len")]
    pub emotional_signal_len: usize,
    #[serde(default = "default_tcs_gradient_signal_len")]
    pub gradient_signal_len: usize,
    #[serde(default = "default_tcs_signal_window_slices")]
    pub signal_window_slices: usize,
    #[serde(default = "default_tcs_max_filtration")]
    pub max_filtration: f32,
    #[serde(default = "default_tcs_persistence_threshold")]
    pub persistence_threshold: f32,
    #[serde(default = "default_tcs_cache_ttl_secs")]
    pub cache_ttl_secs: u64,
    #[serde(default = "default_tcs_cache_capacity")]
    pub cache_capacity: usize,
    #[serde(default = "default_tcs_cache_dir")]
    pub cache_dir: Option<String>,
    #[serde(default = "default_tcs_zero_tolerance")]
    pub zero_tolerance: f64,
    #[serde(default = "default_tcs_laplacian_resolution")]
    pub laplacian_resolution: usize,
    #[serde(default = "default_tcs_max_dimension")]
    pub max_dimension: usize,
    #[serde(default = "default_tcs_enable_gpu")]
    pub enable_gpu: bool,
    #[serde(default)]
    pub approximate_laplacian: bool,
    #[serde(default)]
    pub robust_mode: bool,
    #[serde(default = "default_tcs_retrieval_persistence_weight")]
    pub retrieval_persistence_weight: f32,
    #[serde(default = "default_tcs_retrieval_betti_weight")]
    pub retrieval_betti_weight: f32,
    #[serde(default = "default_tcs_retrieval_entropy_weight")]
    pub retrieval_entropy_weight: f32,
    #[serde(default = "default_tcs_retrieval_anomaly_penalty")]
    pub retrieval_anomaly_penalty: f32,
    #[serde(default = "default_tcs_retrieval_entropy_target")]
    pub retrieval_entropy_target: f32,
    #[serde(default = "default_tcs_retrieval_affinity_floor")]
    pub retrieval_affinity_floor: f32,
    #[serde(default = "default_tcs_retrieval_affinity_ceiling")]
    pub retrieval_affinity_ceiling: f32,
    #[serde(default = "default_tcs_retrieval_similarity_blend")]
    pub retrieval_similarity_blend: f32,
    #[serde(default = "default_tcs_retrieval_route_convergence")]
    pub retrieval_route_convergence: f32,
    #[serde(default = "default_tcs_retrieval_betti_alignment_floor")]
    pub retrieval_betti_alignment_floor: f32,
}

impl Default for TcsRuntimeConfig {
    fn default() -> Self {
        Self {
            takens_dimension: default_tcs_takens_dimension(),
            takens_delay: default_tcs_takens_delay(),
            takens_history_len: default_tcs_takens_history_len(),
            emotional_signal_len: default_tcs_emotional_signal_len(),
            gradient_signal_len: default_tcs_gradient_signal_len(),
            signal_window_slices: default_tcs_signal_window_slices(),
            max_filtration: default_tcs_max_filtration(),
            persistence_threshold: default_tcs_persistence_threshold(),
            cache_ttl_secs: default_tcs_cache_ttl_secs(),
            cache_capacity: default_tcs_cache_capacity(),
            cache_dir: default_tcs_cache_dir(),
            zero_tolerance: default_tcs_zero_tolerance(),
            laplacian_resolution: default_tcs_laplacian_resolution(),
            max_dimension: default_tcs_max_dimension(),
            enable_gpu: default_tcs_enable_gpu(),
            approximate_laplacian: false,
            robust_mode: false,
            retrieval_persistence_weight: default_tcs_retrieval_persistence_weight(),
            retrieval_betti_weight: default_tcs_retrieval_betti_weight(),
            retrieval_entropy_weight: default_tcs_retrieval_entropy_weight(),
            retrieval_anomaly_penalty: default_tcs_retrieval_anomaly_penalty(),
            retrieval_entropy_target: default_tcs_retrieval_entropy_target(),
            retrieval_affinity_floor: default_tcs_retrieval_affinity_floor(),
            retrieval_affinity_ceiling: default_tcs_retrieval_affinity_ceiling(),
            retrieval_similarity_blend: default_tcs_retrieval_similarity_blend(),
            retrieval_route_convergence: default_tcs_retrieval_route_convergence(),
            retrieval_betti_alignment_floor: default_tcs_retrieval_betti_alignment_floor(),
        }
    }
}

impl TcsRuntimeConfig {
    pub fn from_env() -> Self {
        let mut cfg = Self::default();

        if let Some(value) = env_with_fallback(&["TCS_TAKENS_DIM", "TCS_TAKENS_DIMENSION"]) {
            if let Ok(parsed) = value.parse::<usize>() {
                cfg.takens_dimension = parsed.max(1);
            }
        }
        if let Some(value) = env_with_fallback(&["TCS_TAKENS_DELAY"]) {
            if let Ok(parsed) = value.parse::<usize>() {
                cfg.takens_delay = parsed.max(1);
            }
        }
        if let Some(value) = env_with_fallback(&["TCS_TAKENS_HISTORY_LEN"]) {
            if let Ok(parsed) = value.parse::<usize>() {
                cfg.takens_history_len = parsed.max(4);
            }
        }
        if let Some(value) = env_with_fallback(&["TCS_EMOTIONAL_SIGNAL_LEN"]) {
            if let Ok(parsed) = value.parse::<usize>() {
                cfg.emotional_signal_len = parsed.max(4);
            }
        }
        if let Some(value) = env_with_fallback(&["TCS_GRADIENT_SIGNAL_LEN"]) {
            if let Ok(parsed) = value.parse::<usize>() {
                cfg.gradient_signal_len = parsed.max(4);
            }
        }
        if let Some(value) = env_with_fallback(&["TCS_SIGNAL_WINDOW_SLICES"]) {
            if let Ok(parsed) = value.parse::<usize>() {
                cfg.signal_window_slices = parsed.max(1);
            }
        }
        if let Some(value) = env_with_fallback(&["TCS_MAX_FILTRATION"]) {
            if let Ok(parsed) = value.parse::<f32>() {
                cfg.max_filtration = parsed.max(0.1);
            }
        }
        if let Some(value) = env_with_fallback(&["TCS_PERSISTENCE_THRESHOLD"]) {
            if let Ok(parsed) = value.parse::<f32>() {
                cfg.persistence_threshold = parsed.max(0.0);
            }
        }
        if let Some(value) = env_with_fallback(&["TOPOLOGY_CACHE_TTL_SECS"]) {
            if let Ok(parsed) = value.parse::<u64>() {
                cfg.cache_ttl_secs = parsed;
            }
        }
        if let Some(value) = env_with_fallback(&["TOPOLOGY_CACHE_MAX_ENTRIES"]) {
            if let Ok(parsed) = value.parse::<usize>() {
                cfg.cache_capacity = parsed.max(1);
            }
        }
        if let Some(value) = env_with_fallback(&["TOPOLOGY_CACHE_DIR", "TCS_CACHE_DIR"]) {
            let trimmed = value.trim();
            if !trimmed.is_empty() {
                cfg.cache_dir = Some(trimmed.to_string());
            }
        }
        if let Some(value) = env_with_fallback(&["TCS_LAPLACIAN_ZERO_TOLERANCE"]) {
            if let Ok(parsed) = value.parse::<f64>() {
                cfg.zero_tolerance = parsed.max(f64::EPSILON);
            }
        }
        if let Some(value) = env_with_fallback(&["TCS_LAPLACIAN_STEPS"]) {
            if let Ok(parsed) = value.parse::<usize>() {
                cfg.laplacian_resolution = parsed.max(1);
            }
        }
        if let Some(value) = env_with_fallback(&["TCS_LAPLACIAN_MAX_DIM"]) {
            if let Ok(parsed) = value.parse::<usize>() {
                cfg.max_dimension = parsed.min(3);
            }
        }
        if let Some(value) = env_with_fallback(&["TCS_ENABLE_GPU"]) {
            cfg.enable_gpu = matches!(
                value.to_ascii_lowercase().as_str(),
                "1" | "true" | "yes" | "on"
            );
        }
        if let Some(value) = env_with_fallback(&["USE_APPROXIMATE_TDA"]) {
            cfg.approximate_laplacian = matches!(
                value.to_ascii_lowercase().as_str(),
                "1" | "true" | "yes" | "on"
            );
        }
        if let Some(value) = env_with_fallback(&["ROBUST_MODE", "TCS_ROBUST_MODE"]) {
            cfg.robust_mode = matches!(
                value.to_ascii_lowercase().as_str(),
                "1" | "true" | "yes" | "on"
            );
        }
        if let Some(value) = env_with_fallback(&[
            "TCS_RETRIEVAL_PERSISTENCE_WEIGHT",
            "TCS_RETRIEVAL_PERSISTENCE",
        ]) {
            if let Ok(parsed) = value.parse::<f32>() {
                cfg.retrieval_persistence_weight = parsed.max(0.0);
            }
        }
        if let Some(value) =
            env_with_fallback(&["TCS_RETRIEVAL_BETTI_WEIGHT", "TCS_RETRIEVAL_B1_WEIGHT"])
        {
            if let Ok(parsed) = value.parse::<f32>() {
                cfg.retrieval_betti_weight = parsed.max(0.0);
            }
        }
        if let Some(value) =
            env_with_fallback(&["TCS_RETRIEVAL_ENTROPY_WEIGHT", "TCS_RETRIEVAL_ENT_WEIGHT"])
        {
            if let Ok(parsed) = value.parse::<f32>() {
                cfg.retrieval_entropy_weight = parsed.max(0.0);
            }
        }
        if let Some(value) =
            env_with_fallback(&["TCS_RETRIEVAL_ANOMALY_PENALTY", "TCS_RETRIEVAL_H2_PENALTY"])
        {
            if let Ok(parsed) = value.parse::<f32>() {
                cfg.retrieval_anomaly_penalty = parsed.clamp(0.0, 1.0);
            }
        }
        if let Some(value) =
            env_with_fallback(&["TCS_RETRIEVAL_ENTROPY_TARGET", "TCS_RETRIEVAL_ENT_TARGET"])
        {
            if let Ok(parsed) = value.parse::<f32>() {
                cfg.retrieval_entropy_target = parsed.max(f32::EPSILON);
            }
        }
        if let Some(value) =
            env_with_fallback(&["TCS_RETRIEVAL_AFFINITY_FLOOR", "TCS_RETRIEVAL_MIN_AFFINITY"])
        {
            if let Ok(parsed) = value.parse::<f32>() {
                cfg.retrieval_affinity_floor = parsed.max(0.0);
            }
        }
        if let Some(value) = env_with_fallback(&[
            "TCS_RETRIEVAL_AFFINITY_CEILING",
            "TCS_RETRIEVAL_MAX_AFFINITY",
        ]) {
            if let Ok(parsed) = value.parse::<f32>() {
                cfg.retrieval_affinity_ceiling =
                    parsed.max(cfg.retrieval_affinity_floor + f32::EPSILON);
            }
        } else {
            cfg.retrieval_affinity_ceiling = cfg
                .retrieval_affinity_ceiling
                .max(cfg.retrieval_affinity_floor + f32::EPSILON);
        }
        if let Some(value) =
            env_with_fallback(&["TCS_RETRIEVAL_SIMILARITY_BLEND", "TCS_RETRIEVAL_BLEND"])
        {
            if let Ok(parsed) = value.parse::<f32>() {
                cfg.retrieval_similarity_blend = parsed.clamp(0.0, 1.0);
            }
        }
        if let Some(value) =
            env_with_fallback(&["TCS_RETRIEVAL_ROUTE_CONVERGENCE", "TCS_ROUTE_CONVERGENCE"])
        {
            if let Ok(parsed) = value.parse::<f32>() {
                cfg.retrieval_route_convergence = parsed.clamp(0.0, 1.0);
            }
        }
        if let Some(value) =
            env_with_fallback(&["TCS_RETRIEVAL_BETTI_ALIGN_FLOOR", "TCS_B1_ALIGNMENT_FLOOR"])
        {
            if let Ok(parsed) = value.parse::<f32>() {
                cfg.retrieval_betti_alignment_floor = parsed.max(0.5);
            }
        }
    ...
    pub fn robust_mode_enabled(&self) -> bool {
        self.robust_mode
    }
}
...
pub struct RuntimeConfig {
    ...
    #[serde(default = "default_topocot_force_fallback")]
    pub topocot_force_fallback: bool,
...
impl RuntimeConfig {
    pub fn load(args: &CliArgs) -> Result<Self> {
        ...
        let mut vllm_endpoint = env_with_fallback(&vllm_keys)
            .or_else(|| {
                warn!(
                    "Set VLLM_ENDPOINT and ensure Ollama service is running (default http://127.0.0.1:11434)"
                );
                None
            })
            .unwrap_or_else(|| "http://127.0.0.1:11434".to_string())
            ...
            .trim_end_matches('/')
            .to_string();
        // Force Ollama if still pointing to old vLLM endpoint
        if vllm_endpoint.contains("100.113.10.90") || vllm_endpoint.contains(":8000") {
            warn!(
                "Detected old vLLM endpoint, forcing Ollama: {}",
                vllm_endpoint
            );
            vllm_endpoint = "http://127.0.0.1:11434".to_string();
        }
        ...
        let qdrant_primary = env_with_fallback(&["QDRANT_URL"])
            .map(|value| value.trim().trim_end_matches('/').to_string())
            .filter(|value| !value.is_empty());

        let qdrant_url = qdrant_primary
            .clone()
            .or_else(|| {
                env_with_fallback(&["QDRANT_URL_TAILSCALE", "TEST_ENDPOINT_QDRANT"])
                    .map(|value| value.trim().trim_end_matches('/').to_string())
            })
            .unwrap_or_else(|| "http://127.0.0.1:6333".to_string());

        if qdrant_primary.is_none() && qdrant_url.contains("qdrant.io") {
            warn!(%qdrant_url, "QDRANT_URL missing; falling back to cloud-like host from tailscale/test overrides");
        }

        info!(%qdrant_url, "RuntimeConfig using Qdrant endpoint");
        ...
        // Use requested dimension or default to the Qwen3-Embedding-4B output size (2560)
        ...
        let expected_qdrant_dim = 2_560;
        let qdrant_vector_dim = requested_qdrant_dim.unwrap_or(expected_qdrant_dim);

        if let Some(requested) = requested_qdrant_dim {
            if requested != expected_qdrant_dim {
                warn!(
                    requested,
                    expected = expected_qdrant_dim,
                    "Qdrant vector dimension ({requested}) differs from expected ({expected_qdrant_dim}). Ensure embedding model matches."
                );
            }
        }

        let raw_ollama_endpoint =
            env_with_fallback(&["OLLAMA_URL", "OLLAMA_ENDPOINT", "OLLAMA_ENDPOINT_TAILSCALE"])
                .or_else(|| {
                    warn!("Set OLLAMA_URL and run 'ollama serve && ollama pull llama3.1:8b'");
                    None
                })
                .unwrap_or_else(|| "http://127.0.0.1:11434".to_string());
        let ollama_endpoint = raw_ollama_endpoint
            .trim()
            .trim_end_matches('/')
            .replace("/v1/chat/completions", "")
            .replace("/v1/completions", "")
        ...
        let ollama_model =
            env_with_fallback(&["OLLAMA_MAIN_MODEL", "OLLAMA_MODEL", "OLLAMA_MODEL_ID"])
        ...
        .unwrap_or_else(|| "/workspace/models/Qwen3-Embedding-4B-ONNX/model.onnx".to_string());
        ...
        let tcs_config = TcsRuntimeConfig::from_env();
        ...
        let curator_model_name = env_with_fallback(&["CURATOR_MODEL", "CURATOR_MODEL_NAME"])
            .unwrap_or_else(|| "llama3.1:8b".to_string()); // Keep for Ollama curator
        ...
        let topocot_force_fallback = env_with_fallback(&["TOPOCOT_FORCE_FALLBACK"])
            .map(|value| {
                matches!(
                    value.to_ascii_lowercase().as_str(),
                    "1" | "true" | "yes" | "on"
            ...
            .unwrap_or(default_topocot_force_fallback());
        ...
        let mut runtime = Self {
            ...
            use_approximate_tda: tcs_config.approximate_laplacian,
    ...
    /// Active generation endpoint derived from the selected backend.
    pub fn generation_endpoint(&self) -> &str {
        match self.generation_backend {
            BackendType::OllamaCpu => &self.ollama_endpoint,
            _ => &self.vllm_endpoint,
        }
    }

    /// Active generation model identifier derived from the selected backend.
    pub fn generation_model(&self) -> &str {
        match self.generation_backend {
            BackendType::OllamaCpu => &self.ollama_model,
            _ => &self.vllm_model,
        }
    }
}
...
fn load_env_file(path: &Path) -> Result<()> {
    ...
    for (_line_index, line) in contents.lines().enumerate() {
        ...
        // Allow shell-style "export KEY=VALUE" entries in env files by stripping leading "export".
        let raw_key = parts.next().unwrap_or("").trim();
        let key = if raw_key.to_lowercase().starts_with("export ") {
            raw_key[7..].trim()
        } else {
            raw_key
        };
...
fn default_training_queue_path() -> String {
    env_with_fallback(&["TRAINING_QUEUE_PATH"]).unwrap_or_else(|| "data/training_queue".to_string())
}
...
fn default_learning_timeout_secs() -> u64 {
    ...
    env_with_fallback(&["LEARNING_TIMEOUT_SECS", "LEARNING_TIMEOUT"])
        .and_then(|value| value.parse().ok())
        .unwrap_or(30) // Increased from 20 to 30 seconds to prevent premature timeouts
}
...
fn default_generation_client_timeout_secs() -> u64 {
    env_with_fallback(&["GENERATION_CLIENT_TIMEOUT_SECS", "GENERATION_TIMEOUT_SECS"])
        .and_then(|value| value.parse().ok())
        .unwrap_or(60)
}
...
fn default_tcs_takens_dimension() -> usize {
    3
}

fn default_tcs_takens_delay() -> usize {
    2
}

fn default_tcs_takens_history_len() -> usize {
    24
}

fn default_tcs_emotional_signal_len() -> usize {
    32
}

fn default_tcs_gradient_signal_len() -> usize {
    32
}

fn default_tcs_signal_window_slices() -> usize {
    4
}

fn default_tcs_max_filtration() -> f32 {
    1.5
}

fn default_tcs_persistence_threshold() -> f32 {
    0.8
}

fn default_tcs_cache_ttl_secs() -> u64 {
    900
}

fn default_tcs_cache_capacity() -> usize {
    1_024
}

fn default_tcs_cache_dir() -> Option<String> {
    Some("storage/topology_cache".to_string())
}

fn default_tcs_zero_tolerance() -> f64 {
    1.0e-6
}

fn default_tcs_laplacian_resolution() -> usize {
    8
}

fn default_tcs_max_dimension() -> usize {
    2
}

fn default_tcs_enable_gpu() -> bool {
    true
}

fn default_tcs_retrieval_persistence_weight() -> f32 {
    0.5
}

fn default_tcs_retrieval_betti_weight() -> f32 {
    0.3
}

fn default_tcs_retrieval_entropy_weight() -> f32 {
    0.2
}

fn default_tcs_retrieval_anomaly_penalty() -> f32 {
    0.35
}

fn default_tcs_retrieval_entropy_target() -> f32 {
    1.2
}

fn default_tcs_retrieval_affinity_floor() -> f32 {
    0.1
}

fn default_tcs_retrieval_affinity_ceiling() -> f32 {
    1.5
}

fn default_tcs_retrieval_similarity_blend() -> f32 {
    0.35
}

fn default_tcs_retrieval_route_convergence() -> f32 {
    0.6
}

fn default_tcs_retrieval_betti_alignment_floor() -> f32 {
    1.0
}
