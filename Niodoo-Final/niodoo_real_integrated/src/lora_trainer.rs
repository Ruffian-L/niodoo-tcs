...
impl LoRAAdapter {
    ...
    pub fn new(config: LoRAConfig) -> Result<Self> {
        ...
        let lora_b_dtype = if config.use_fp16 {
            DType::F16
        } else {
            DType::F32
        };
    ...
    pub fn save_adapter<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        ...
        let (lora_a_bytes, lora_a_dtype, lora_a_shape) = if use_fp16 {
            ...
            let lora_a_f16: Vec<u16> = lora_a_flat
                .iter()
                .map(|f| f16::from_f32(*f).to_bits())
                .collect();
            let lora_a_bytes: Vec<u8> = lora_a_f16
                .iter()
        ...
        let (lora_b_bytes, lora_b_dtype, lora_b_shape) = if use_fp16 {
            ...
            let lora_b_f16: Vec<u16> = lora_b_flat
                .iter()
                .map(|f| f16::from_f32(*f).to_bits())
                .collect();
            let lora_b_bytes: Vec<u8> = lora_b_f16
                .iter()
                .flat_map(|bits| bits.to_le_bytes())
                .collect();
            (
        ...
        let lora_a_view =
            safetensors::tensor::TensorView::new(lora_a_dtype, lora_a_shape, &lora_a_bytes)?;
        tensors.insert("lora_a".to_string(), lora_a_view);

        // Create lora_b TensorView
        let lora_b_view =
            safetensors::tensor::TensorView::new(lora_b_dtype, lora_b_shape, &lora_b_bytes)?;
        ...
        let lora_a = match lora_a_tensor.dtype() {
            safetensors::Dtype::F16 => {
                ...
                if lora_a_bytes.len() % 2 != 0 {
                    return Err(anyhow!(
                        "lora_a byte length ({}) is not a multiple of 2 bytes (f16 size)",
                        lora_a_bytes.len()
                    ));
                }
            ...
            safetensors::Dtype::F32 => {
                ...
                if lora_a_bytes.len() % 4 != 0 {
                    return Err(anyhow!(
                        "lora_a byte length ({}) is not a multiple of 4 bytes (f32 size)",
                        lora_a_bytes.len()
                    ));
                }
        ...
        let lora_b = match lora_b_tensor.dtype() {
            safetensors::Dtype::F16 => {
                ...
                if lora_b_bytes.len() % 2 != 0 {
                    return Err(anyhow!(
                        "lora_b byte length ({}) is not a multiple of 2 bytes (f16 size)",
                        lora_b_bytes.len()
                    ));
                }
            ...
            safetensors::Dtype::F32 => {
                ...
                if lora_b_bytes.len() % 4 != 0 {
                    return Err(anyhow!(
                        "lora_b byte length ({}) is not a multiple of 4 bytes (f32 size)",
                        lora_b_bytes.len()
                    ));
                }
...
/// Captures diagnostics from the latest gradient step for telemetry.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LoRAUpdateStats {
    pub grad_a_norm: f32,
    pub grad_b_norm: f32,
...
pub struct LoRATrainer {
    ...
    config: LoRAConfig,
    /// Latest gradient diagnostics (if any)
    last_update_stats: Option<LoRAUpdateStats>,
}
...
impl LoRATrainer {
    ...
    /// Return diagnostics from the latest gradient update.
    pub fn last_update_stats(&self) -> Option<LoRAUpdateStats> {
        self.last_update_stats.clone()
    }

    fn record_update_stats(
        &mut self,
        grad_a_norm: f32,
        grad_b_norm: f32,
        learning_rate: f32,
        ...
        let scaled_a = grad_a_norm * learning_rate;
        let scaled_b = grad_b_norm * learning_rate;
        let weight_delta_norm = (scaled_a * scaled_a + scaled_b * scaled_b).sqrt();
        self.last_update_stats = Some(LoRAUpdateStats {
            grad_a_norm,
            grad_b_norm,
            weight_delta_norm,
            loss,
...
impl LoRATrainer {
    ...
    pub fn train_batch(
        ...
        if loss_val > 0.001 {
            ...
            let grad_a_intermediate = batched_input.transpose(0, 1)?.matmul(&grad_output_scaled)?;
    ...
    pub fn train_epoch(
        ...
        if data.is_empty() {
            ...
            return Err(anyhow::anyhow!(
                "Training loop executed but no data provided"
            ));
        }
        ...
        if batch_count == 0 {
            ...
            return Err(anyhow::anyhow!(
                "Training loop executed but no batches processed"
            ));
        }
        ...
        let weight_diff_a: f64 = initial_weight_a
            ...
            .map(|(init_row, fin_row)| {
                init_row
                    .iter()
                    .zip(fin_row.iter())
                    .map(|(i, f)| ((i - f) as f64).abs())
                    .sum::<f64>()
            })
            .sum();
    ...
    pub fn train(
        ...
        tracing::info!(
            "🔍 DIAGNOSTIC: Starting training with {} epochs, {} samples",
            epochs,
            data.len()
        );
        ...
        for epoch in 0..epochs {
            ...
            tracing::info!(
                "🔍 DIAGNOSTIC: train_epoch called for epoch {} (lr: {:.6})",
                epoch,
                current_lr
            );
            ...
            if batch_count == 0 {
                ...
                return Err(anyhow::anyhow!(
                    "Training loop executed but no batches processed in epoch {}",
                    epoch
                ));
            }
            ...
            if total_loss > 0.001 {
                ...
                for (batch_start, batch_end) in batch_ranges {
                    ...
                    if loss_val > 0.001 {
                        ...
                        let grad_a_norm =
                            grad_a_clipped.sqr()?.sum_all()?.to_scalar::<f32>()?.sqrt();
                        let grad_b_norm =
                            grad_b_clipped.sqr()?.sum_all()?.to_scalar::<f32>()?.sqrt();
                        self.record_update_stats(
                            grad_a_norm,
                            grad_b_norm,
                            current_lr,
                ...
                tracing::warn!(
                    "Skipping gradient updates: total_loss ({:.6}) <= 0.001",
                    total_loss
                );
            }
