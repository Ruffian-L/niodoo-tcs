...
impl TorusPadMapper {
    ...
    /// Map a 2560-dimensional embedding onto the 7D PAD+ghost manifold.
    #[instrument(skip_all)]
    pub fn project(&mut self, embedding: &[f32]) -> Result<PadGhostState> {
        ...
        // Softplus for variance to keep positive, with small clamp to avoid collapse.
        let mut sigma = [0.5f64; 7];

        // Reparameterisation trick with deterministic RNG.
        let mut pad = [0.0f64; 7];
        for i in 0..7 {
            let raw_var = softplus(logvar[i]);
            let sigma_val = (raw_var + 1e-6).sqrt().clamp(0.05, 3.0);
            sigma[i] = sigma_val;
