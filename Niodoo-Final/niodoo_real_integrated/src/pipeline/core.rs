...
use crate::erag::{CollapseResult, EmotionalVector, EragClient};
...
use crate::pipeline::state::{
    CuratedExperience, CuratorFeedbackController, PipelineCycle, Thresholds, TorusSeedStrategy,
};
...
pub struct Pipeline {
    ...
    pub(crate) telemetry_tx:
        Option<tokio::sync::broadcast::Sender<crate::telemetry::CognitiveStatePacket>>,
    // Telemetry: file logger for enhanced packet playback
    pub(crate) file_logger: Option<Arc<crate::telemetry::file_logger::FileLogger>>,
    // Telemetry: iteration counter
    pub(crate) iteration_count: Arc<AtomicU64>,
    // Gemini API client for Learning Gate failure review
    pub(crate) gemini_client: Option<reqwest::Client>,
    pub(crate) gemini_api_key: Option<String>,
    // Pending topology enrichment signals (fed into TCS on next cycle)
    pending_topology_emotions: Vec<EmotionalVector>,
    pending_lora_gradients: Vec<f32>,
}
...
impl Pipeline {
    ...
    async fn initialise_with_topology(
        ...
        let resolved_vector_dim = embedder.vector_dim();
        if resolved_vector_dim != config.qdrant_vector_dim {
            info!(
                previous = config.qdrant_vector_dim,
                resolved = resolved_vector_dim,
                "Detected embedding dimension from model; updating runtime configuration"
            );
            config.qdrant_vector_dim = resolved_vector_dim;
            let resolved_str = resolved_vector_dim.to_string();
            set_env_override("QDRANT_VECTOR_DIM", resolved_str.clone());
            set_env_override("QDRANT_VECTOR_SIZE", resolved_str.clone());
            set_env_override("NIODOO_EMBED_DIM", resolved_str);
        }
        ...
        let compass = {
            ...
            let compass_config = CompassConfig {
                ...
                quadrant_persist_arousal_threshold: config
                    .compass_quadrant_persist_arousal_threshold,
                ...
                cascade_recognition_satisfaction_consonance: config
                    .compass_cascade_recognition_satisfaction_consonance,
                cascade_calm_motivation_consonance: config
                    .compass_cascade_calm_motivation_consonance,
            };
        ...
        let tokenizer = Arc::new(
            DynamicTokenizerManager::initialise(
                ...
                config.mirage_sigma_factor * stats.entropy_mean,
            )
            .await?,
        );
        tokenizer.spawn_maintenance().await;
        let generation_http_timeout = config
            .generation_client_timeout_secs
            .max(config.generation_timeout_secs);
        ...
        info!(
            ...
            endpoint = %config.generation_endpoint(),
            timeout_secs = generation_http_timeout,
            "Initialized HTTP generation engine"
        );
        ...
        if let Err(error) = learning.spawn_async_trainer() {
            warn!(
                %error,
                "Failed to start async LoRA trainer; continuing with synchronous path"
            );
        }
        ...
        let tcs_analyzer = if matches!(config.topology_mode, TopologyMode::Hybrid) {
            match TCSAnalyzer::new_with_runtime(&config.tcs) {
                Ok(analyzer) => {
                    info!(
                        "TCS topology analyzer initialized (approximate_tda: {}, robust: {})",
                        config.tcs.approximate_laplacian, config.tcs.robust_mode
                    );
                    Some(analyzer)
                }
                Err(error) => {
        ...
        let topology_analyzer = Arc::new(TopologyMemoryAnalyzer::new(
            config.topology_memory_analyzer_threshold as f32,
        ));
        ...
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(Duration::from_secs(
                config.gpu_fitness_refresh_interval_secs,
            ));
            loop {
                interval.tick().await;
        ...
        let (telemetry_tx, telemetry_rx) = if config.telemetry_enabled {
            let (tx, rx) =
                tokio::sync::broadcast::channel::<crate::telemetry::CognitiveStatePacket>(16);
        ...
        // Initialize file logger for enhanced packet playback
        let file_logger = if config.telemetry_enabled {
            match crate::telemetry::file_logger::FileLogger::new(
                crate::telemetry::file_logger::FileLoggerConfig {
                    log_dir: std::path::PathBuf::from("logs/telemetry"),
                    max_file_size: 100 * 1024 * 1024, // 100 MB
                    max_files: 10,
                    compress: true,
                    file_prefix: "niodoo_telemetry".to_string(),
                },
            )
            .await
            {
                Ok(logger) => {
                    info!("File logger initialized for telemetry playback");
                    Some(Arc::new(logger))
                }
                Err(e) => {
                    warn!(error = %e, "Failed to initialize file logger, continuing without playback");
                    None
                }
            }
        } else {
        ...
        Ok(Self {
            ...
            file_logger,
            iteration_count,
            // Initialize Gemini client for Learning Gate
            gemini_client: Some(reqwest::Client::new()),
            gemini_api_key: Some("AIzaSyBVAFFVXbze-CxdVnbaL3Gk0o93L4pXIzk".to_string()),
            pending_topology_emotions: Vec::new(),
            pending_lora_gradients: Vec::new(),
        })
    }

    /// Phase 4.2: Helper to adjust runtime parameters based on curator feedback
    pub(crate) fn adjust_runtime_param(config: &mut RuntimeConfig, param: &str, delta: f64) {
        match param {
            "temperature" => {
                config.temperature = (config.temperature + delta)
                    .clamp(config.pipeline_param_min, config.pipeline_param_max);
            }
            "top_p" => {
                config.top_p = (config.top_p + delta)
                    .clamp(config.pipeline_param_min, config.pipeline_param_max);
            }
            "retrieval_top_k" => {
                let updated = (config.phase2_retrieval_top_k_increment as f64 + delta).clamp(
                    config.pipeline_retrieval_top_k_increment_min,
                    config.pipeline_retrieval_top_k_increment_max,
                );
    ...
    pub fn set_topology_mode(&mut self, mode: TopologyMode) -> Result<()> {
        ...
        self.tcs_analyzer = match mode {
            TopologyMode::Hybrid => match TCSAnalyzer::new_with_runtime(&self.config.tcs) {
                Ok(analyzer) => {
                    info!(
                            "TCS analyzer re-initialized for hybrid mode (approximate_tda: {}, robust: {})",
                            self.config.tcs.approximate_laplacian,
                            self.config.tcs.robust_mode
                        );
                    Some(analyzer)
                }
                Err(error) => {
...
fn baseline_topological_signature(
    ...
    let takens_energy = 0.0;
    let takens_window_count = 0;
    let takens_window_energy = Vec::new();
    let emotional_flux = 0.0;
    let emotional_drift = 0.0;
    let emotional_arc_lengths = Vec::new();
    let gradient_energy = laplacian_spectral_radius;
    let gradient_volatility = 0.0;
    let gradient_energy_trend = Vec::new();
...
impl Pipeline {
    pub(crate) fn drain_pending_signals(&mut self) -> (Vec<EmotionalVector>, Vec<f32>) {
        let emotions = std::mem::take(&mut self.pending_topology_emotions);
        let gradients = std::mem::take(&mut self.pending_lora_gradients);
        (emotions, gradients)
    }

    pub(crate) fn set_pending_topology_emotions(&mut self, signals: Vec<EmotionalVector>) {
        self.pending_topology_emotions = signals;
    }

    pub(crate) fn set_pending_lora_gradients(&mut self, gradients: Vec<f32>) {
        self.pending_lora_gradients = gradients;
    }
}
