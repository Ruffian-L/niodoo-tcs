...
impl DocPuzzleRunner {
    ...
    pub fn load_tasks<P: AsRef<std::path::Path>>(
        path: P,
    ) -> Result<Vec<DocPuzzleTask>, anyhow::Error> {
    ...
    fn evaluate_checklist(&self, task: &DocPuzzleTask, steps: &[String]) -> f64 {
        ...
        let matches = task
            ...
            .filter(|required| {
                ...
                required_lower
                    .split_whitespace()
                    .any(|word| word.len() > 3 && steps_text.contains(word))
            })
            .count();
        ...
        >,
    {
        ...
        let avg_process_score =
            results.iter().map(|r| r.process_score).sum::<f64>() / results.len() as f64;
        let avg_answer_correctness =
            results.iter().map(|r| r.answer_correctness).sum::<f64>() / results.len() as f64;
        let avg_checklist_compliance =
            results.iter().map(|r| r.checklist_compliance).sum::<f64>() / results.len() as f64;
