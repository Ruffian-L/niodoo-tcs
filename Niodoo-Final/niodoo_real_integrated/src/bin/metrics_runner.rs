...
async fn main() -> Result<()> {
    ...
    info!(
        "🚀 Starting metrics runner with scenario: {:?}",
        args.scenario
    );
    ...
    let report = match args.scenario {
        ...
        Scenario::Baseline => run_baseline(pipeline).await?,
        Scenario::Cognitive => run_cognitive_baseline(pipeline).await?,
    };
...
fn main() {
    eprintln!("Enable the `cli_bins` feature to run `metrics_runner`.");
}
...
pub async fn run_load_test(
    ...
    for user_id in 0..concurrent_users {
        ...
        let request_count = request_counter.clone();
        let errors_clone = errors_ref.clone();
            ...
            while Instant::now() < end_time {
                ...
                match result {
                    Ok(cycle) => {
                        ...
                        m.record_stage_latency("erag".to_string(), cycle.stage_timings.erag_ms);
                        if cycle.stage_timings.tcs_ms > 0.0 {
                            m.record_stage_latency("tcs".to_string(), cycle.stage_timings.tcs_ms);
                        }
                    }
                    Err(e) => {
                        errors_clone
                            .lock()
                            .await
                            .push(format!("User {} error: {}", user_id, e));
                    }
                }
