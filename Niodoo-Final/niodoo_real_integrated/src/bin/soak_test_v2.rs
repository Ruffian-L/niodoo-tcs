...
#[cfg(all(feature = "cli_bins", feature = "soak_harness"))]
#[path = "soak_test_v2/soak_prompts_v2.rs"]
mod soak_prompts_v2;
...
impl SoakMetrics {
    ...
    async fn stats(&self) -> SoakStats {
        ...
        let (avg_memory, memory_growth) = if memory_samples.len() >= 2 {
            ...
            let growth = samples
                .last()
                .expect("samples.len() >= 2 ensures last() exists")
                - samples
                    .first()
                    .expect("samples.len() >= 2 ensures first() exists");
            (avg, growth)
        } else {
            (0.0, 0.0)
        };
...
async fn configure_services() {
    let vllm_endpoint =
        std::env::var("VLLM_ENDPOINT").unwrap_or_else(|_| "http://127.0.0.1:11434".to_string());
    let ollama_url =
        std::env::var("OLLAMA_URL").unwrap_or_else(|_| "http://127.0.0.1:11434".to_string());
    let qdrant_url =
        std::env::var("QDRANT_URL").unwrap_or_else(|_| "http://127.0.0.1:6333".to_string());

    let vllm_port = extract_port(&vllm_endpoint, "11434");
