...
#[cfg(all(feature = "svc", feature = "cli_bins"))]
#[tokio::main]
async fn main() -> Result<()> {
    ...
    let job_queue = Arc::new(JobQueue::new(&args.queue_dir).context("Failed to create job queue")?);

    // Initialize adapter storage
    let adapter_storage = Arc::new(
        AdapterStorage::new(&args.storage_dir).context("Failed to create adapter storage")?,
    );

    // Start worker threads
    let mut worker_handles = Vec::new();
    for i in 0..args.workers {
        let worker = TrainingWorker::new(Arc::clone(&job_queue), Arc::clone(&adapter_storage));
...
#[cfg(not(all(feature = "svc", feature = "cli_bins")))]
fn main() {
    eprintln!("Training service requires the `svc` and `cli_bins` features to be enabled.");
    eprintln!("Build with: cargo build --features svc,cli_bins --bin training_service");
    std::process::exit(1);
}
