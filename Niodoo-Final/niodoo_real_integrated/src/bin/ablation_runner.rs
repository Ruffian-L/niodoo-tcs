...
impl AblationRunner {
    ...
    pub async fn run_experiment(
        ...
        let pipeline = Pipeline::initialise(cli_args)
            .await
        ...
        let mut workers = Vec::with_capacity(concurrent_users);
            ...
            workers.push(async move {
                ...
                while Instant::now() < end_time {
                    ...
                    let result = {
                        ...
                        pipeline_guard.process_prompt(prompt.as_str()).await
                    };
                    ...
                    match result {
                        Ok(_cycle) => {
                            ...
                            {
                                let mut guard = latencies_clone.lock().await;
        ...
        let latency_stats = StatisticalSummary::from_values(&latencies_vec);
    ...
    pub fn compare_with_baseline(
        ...
        let throughput_change = if baseline_throughput > 0.0 {
            (ablation.metrics.throughput - baseline_throughput) / baseline_throughput * 100.0
        } else {
            0.0
        };
...
async fn main() -> Result<()> {
    ...
    if let Some(baseline_path) = &args.baseline {
        ...
        let baseline_throughput = baseline["throughput"]["tokens_per_sec"]
            .as_f64()
            .unwrap_or(0.0);
    ...
    let output_file = args
        .output_dir
        .join(format!("ablation-{:?}.json", args.experiment));
    ...
    if let Some(ref comparison) = result.comparison {
        if comparison.regression_detected {
            eprintln!(
                "⚠️  REGRESSION DETECTED: Latency increased by {:.2}ms ({:.1}%)",
                comparison.latency_change_p99_ms, comparison.latency_change_pct
            );
        } else {
...
fn main() {
    eprintln!("Enable the `cli_bins` feature to run `ablation_runner`.");
}
