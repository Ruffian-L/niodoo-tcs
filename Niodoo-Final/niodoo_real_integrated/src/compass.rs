...
impl CompassEngine {
    ...
    pub fn evaluate_with_ntoken(
        ...
        if let Some(ntoken) = ntoken_features {
            ...
            let h1_persistence_norm =
                (ntoken.h1_total_persistence / cfg.h1_persistence_divisor).tanh(); // Maps to [-1, 1]
            ...
            let sheaf_boost = if ntoken.sheaf_energy < cfg.sheaf_energy_threshold {
                (cfg.sheaf_energy_threshold - ntoken.sheaf_energy) * cfg.sheaf_boost_multiplier
            // Stronger boost for lower energy
            } else {
                0.0
            };
            ...
            dominance = (dominance - h1_penalty * cfg.dominance_penalty_multiplier
                + sheaf_boost * cfg.dominance_boost_multiplier)
                .clamp(-1.0, 1.0);
        ...
        pleasure = (pleasure
            + self
                ._rng
                .gen_range(-cfg.random_noise_range..cfg.random_noise_range))
        .clamp(-1.0, 1.0);
        arousal = (arousal
            + self
                ._rng
        ...
        if !is_threat && self._rng.gen_bool(cfg.random_threat_probability) {
            if arousal > cfg.random_threat_arousal_threshold
                && pleasure < cfg.random_threat_pleasure_threshold
            {
                is_threat = true;
            }
        }

        let is_healing = pleasure > cfg.healing_pleasure_threshold
            && dominance > cfg.healing_dominance_threshold;

        let quadrant = match (pleasure, arousal) {
            (p, a)
                if p < cfg.quadrant_panic_pleasure_threshold
                    && a > cfg.quadrant_panic_arousal_threshold =>
            {
                CompassQuadrant::Panic
            }
            (p, a) if p < 0.0 && a <= cfg.quadrant_persist_arousal_threshold => {
        ...
        match (self.last_quadrant, self.last_entropy) {
            (Some(prev), Some(prev_entropy)) => {
                ...
                let base = match (prev, quadrant) {
                    ...
                    | (CompassQuadrant::Panic, CompassQuadrant::Master) => {
                        cfg.reward_panic_to_discover
                    }
                    (CompassQuadrant::Panic, CompassQuadrant::Persist) => {
                        cfg.reward_panic_to_persist
                    }
    ...
    fn compute_topology_bonus(
        ...
        match strategy {
            "unwind_loops" => {
                ...
                let h1_bonus = (topology.betti_numbers[1] as f64).min(cfg.mcts_h1_bonus_cap)
                    * cfg.mcts_h1_bonus_multiplier;
                let persistence_bonus = (topology.total_persistence / cfg.mcts_persistence_divisor)
                    .min(1.0)
                    * cfg.mcts_persistence_multiplier;
                h1_bonus + persistence_bonus
            }
            "simplify_structure" => {
                // Bonus for high knot complexity
                let knot_bonus = (topology.knot_complexity * cfg.mcts_knot_multiplier)
                    .min(cfg.mcts_knot_multiplier_cap)
                    * cfg.mcts_knot_weight;
                knot_bonus
            }
            "stabilize" => {
            ...
            "structure" => {
                ...
                let entropy_bonus = (topology.persistence_entropy * cfg.mcts_entropy_multiplier)
                    .min(cfg.mcts_entropy_multiplier_cap)
                    * cfg.mcts_entropy_weight;
                entropy_bonus
            }
            "connect" => {
                // Bonus for high H₀ (many disconnected components)
                let h0_bonus = (topology.betti_numbers[0] as f64).min(cfg.mcts_h0_bonus_cap)
                    * cfg.mcts_h0_bonus_multiplier;
                h0_bonus
            }
            _ => {
                // Default: small exploration bonus
                cfg.mcts_default_exploration_base * (branch_idx as f64 + 1.0)
                    / cfg.mcts_default_exploration_divisor
            }
        }
    }
    ...
    pub fn evaluate_with_rng(
        ...
        pleasure = (pleasure + rng.gen_range(-cfg.random_noise_range..cfg.random_noise_range))
            .clamp(-1.0, 1.0);
        arousal = (arousal + rng.gen_range(-cfg.random_noise_range..cfg.random_noise_range))
            .clamp(-1.0, 1.0);
        dominance = (dominance + rng.gen_range(-cfg.random_noise_range..cfg.random_noise_range))
            .clamp(-1.0, 1.0);
