...
use crate::data::{
    ...
    EmotionalSample, Experience, RutPrompt,
};
...
use crate::metrics::{cache_metrics, metrics, FailureSignals};
...
pub struct Pipeline {
    ...
    embedding_cache: Arc<AsyncMutex<LruCache<u64, EmbeddingCacheEntry>>>,
    collapse_cache: Arc<AsyncMutex<LruCache<u64, CollapseCacheEntry>>>,
...
impl Pipeline {
    ...
    async fn initialise_with_topology(
        ...
        info!(path = %tokenizer_file.display(), "initializing legacy dynamic tokenizer");
        ...
        let embedding_cache = Arc::new(AsyncMutex::new(LruCache::new(cache_capacity)));
        let collapse_cache = Arc::new(AsyncMutex::new(LruCache::new(cache_capacity)));
        ...
        let embedder_for_prefetch = embedder.clone();
        ...
        let mut pipeline = Self {
            ...
            collapse_cache: collapse_cache.clone(),
            retry_count: Arc::new(AtomicU32::new(0)),
            qdrant_process,
        };

        if let Some(prompts) = prefetch_prompts {
            Self::spawn_cache_prefetch(
...
fn baseline_topological_signature(
    ...
    let persistence_deltas: Vec<f64> = persistence_features
        .iter()
        .map(|pf| (pf.death - pf.birth) as f64)
        .collect();
