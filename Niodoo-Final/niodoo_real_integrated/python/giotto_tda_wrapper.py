...
"""
import numpy as np
# Note: The pip package 'giotto-tda' actually installs a module named 'gtda'
# This is a known naming mismatch - the package was renamed but pip name stayed for compatibility
try:
    from gtda.homology import VietorisRipsPersistence
except ImportError:
    # Fallback for older installations or if someone installs a different package
    try:
        from giotto_tda.homology import VietorisRipsPersistence
    except ImportError:
        raise ImportError(
            "Neither gtda nor giotto_tda is available. "
            "Install with: pip install giotto-tda "
            "(Note: pip package 'giotto-tda' installs module 'gtda')"
        )
