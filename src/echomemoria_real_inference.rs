...
#[cfg(feature = "onnx")]
// use ort::{Session, Value, GraphOptimizationLevel, LoggingLevel, execution_providers::ExecutionProvider}; // Temporarily disabled
...
pub struct RealMemoryProcessor {
    // session: ort::Session, // Temporarily disabled
...
impl Default for RealEmotionDetector {
    fn default() -> Self {
        ...
        // Load model path from environment or use default model directory
        let model_path = env::var("NIODOO_MODEL_PATH")
            .unwrap_or_else(|_| {
                let home = env::var("HOME").unwrap_or_else(|_| "/tmp".to_string());
                format!("{}/niodoo-models/bert-emotion.onnx", home)
            });
...
impl RealEmotionDetector {
    ...
    pub fn new(config: EmotionConfig) -> Result<Self> {
        #[cfg(feature = "onnx")]
        {
            // use ort::{Environment, SessionBuilder}; // Temporarily disabled
        ...
        {
            use crate::error::NiodooError;

            error!("ONNX feature not enabled - cannot create real emotion detector");
            error!("Enable with: cargo build --features onnx");
            Err(NiodooError::EmotionDetectionError(
                "ONNX feature is required for real emotion detection. Build with --features onnx".to_string()
            ).into())
        }
    }
    ...
    pub async fn detect_emotion(&self, text: &str) -> Result<RealInferenceResult> {
        ...
        #[cfg(not(feature = "onnx"))]
        {
            ...
            error!("Cannot perform emotion detection without ONNX feature enabled");
            Err(NiodooError::EmotionDetectionError(
                "ONNX feature required. Build with: cargo build --features onnx".to_string()
            ).into())
        }
    }
    ...
    fn parse_emotion_output(&self, outputs: &[Value]) -> Result<EmotionResult> {
        #[cfg(feature = "onnx")]
        {
            if outputs.is_empty() {
        ...
        #[cfg(not(feature = "onnx"))]
        {
            Err(anyhow!("ONNX feature not enabled, cannot parse emotion output"))
        }
    }
}
...
impl RealMemoryProcessor {
    ...
    pub fn new(config: MemoryConfig) -> Result<Self> {
        anyhow::bail!("ONNX Runtime disabled: ort crate removed for this build")
    }
    ...
    /// Generate memory insights using ONNX model
    fn generate_memory_insights(&self, embedding: &[f32], context: &str) -> Result<String> {
...
impl RealConsciousnessSynthesizer {
    ...
    pub fn new(config: EchoMemoriaConfig) -> Result<Self> {
        let emotion_config = EmotionConfig {
            model_path: "models/bert-emotion.onnx".to_string(),
            tokenizer_path: "models/bert-tokenizer.json".to_string(),
            max_length: 128,
            emotion_threshold: crate::utils::threshold_convenience::emotion_threshold(),
            batch_size: 1,
        };

        let memory_config = MemoryConfig {
            model_path: "models/gaussian-memory.onnx".to_string(),
            gaussian_dimensions: 512,
            memory_threshold: crate::utils::threshold_convenience::memory_threshold(),
            traversal_steps: 100,
        };
...
/// Simple BERT tokenizer (placeholder for real implementation)
...
impl<RealInferenceResult> RealEchoMemoria<RealInferenceResult> {
    /// Create a new real EchoMemoria system
    pub fn new() -> Result<Self> {
        let config = EchoMemoriaConfig {
...
pub async fn demo_real_echomemoria() -> Result<()> {
    ...
    error!("Cannot run EchoMemoria demo without ONNX feature");
    error!("Build with: cargo build --features onnx --release");
    Err(NiodooError::EmotionDetectionError(
        "ONNX feature required for EchoMemoria demo".to_string()
    ).into())
}
...
mod tests {
    ...
    async fn test_real_emotion_detection() {
        let config = EmotionConfig {
            model_path: "models/test-emotion.onnx".to_string(),
            tokenizer_path: "models/test-tokenizer.json".to_string(),
            max_length: 128,
