...
use crate::embeddings::EmbeddingEngine;  // Assume torus emotional embed
use crate::compass::Compass;  // MCTS decision
...
/// Pipeline Context - Centralized state management for Niodoo pipeline
#[derive(Debug, Clone)]
pub struct PipelineContext {
    ...
    pub traversal_result: Option<serde_json::Value>,
    pub retrieved_memories: Vec<(String, f32)>,
    pub generated_response: String,
    pub metrics: PipelineMetrics,

    // Caching (L1 in-memory)
    pub embedding_cache: HashMap<String, Vec<f32>>,
    pub context_cache: HashMap<String, serde_json::Value>,
}
...
impl PipelineContext {
    ...
    pub fn cache_embedding(&mut self, text: &str, embedding: Vec<f32>) {
        let key = self.get_cache_key(text);
    ...
    pub fn to_json_output(&self) -> String {
        let output = json!({
            "response": self.generated_response,
            "metrics": self.metrics.to_json(),
            "emotional_context": self.emotional_context,
            "memory_count": self.retrieved_memories.len(),
            "traversal_position": self.traversal_result
        });
        output.to_string()
    }

    pub fn log_stage_time(&mut self, stage: &str, start: Instant) {
        let elapsed = start.elapsed().as_millis() as f32;
        match stage {
            "embedding" => self.metrics.embedding_time_ms = elapsed,
            "traversal" => self.metrics.traversal_time_ms = elapsed,
            "retrieval" => self.metrics.retrieval_time_ms = elapsed,
            "generation" => self.metrics.generation_time_ms = elapsed,
            _ => {}
        }
    }
}
...
pub struct PipelineMetrics {
    ...
    pub rouge_score: f32,  // Stub for self-consistency
}
...
impl PipelineMetrics {
    ...
    pub fn to_json(&mut self) -> serde_json::Value {
        ...
        json!({
            ...
            "traversal_time_ms": self.traversal_time_ms,
...
impl ConsciousnessPipelineOrchestrator {
    ...
    /// Full NIODOO-TCS Pipeline Process (Week 1 Integration)
    pub async fn niodoo_process(&self, prompt: &str, context: &mut PipelineContext) -> Result<PipelineOutput> {
        let start = Instant::now();
        context.input_prompt = prompt.to_string();
        info!("🚀 NIODOO-TCS Pipeline: Processing '{}'", &prompt[..50.min(prompt.len())]);

        // Initialize components (assume global or injected)
        let embedder = EmbeddingEngine::new()?;  // Torus emotional embed
        let mut compass = Compass::new();  // MCTS decision
        let erag = EragMemory::new().await?;  // Qdrant retrieval

        // STAGE 1: EMOTIONAL MAPPING (torus, 45ms target)
        let embed_start = Instant::now();
        let emotional_valence = self.assess_emotion_stage(&PipelineInput { text: prompt.to_string(), ..Default::default() }).await?.intensity as f32;
        let embedding = if let Some(cached) = context.get_cached_embedding(prompt) {
            cached
        } else {
            let new_emb = embedder.embed_text_emotional(prompt, emotional_valence).await?;
            context.cache_embedding(prompt, new_emb.clone());
            new_emb
        };
        context.embedding = Some(embedding.clone());
        context.emotional_context = emotional_valence;
        context.log_stage_time("embedding", embed_start);

        // STAGE 2: PARALLEL COMPASS + ERAG (205ms target)
        let decision_start = Instant::now();
        let (compass_decision, erag_docs) = tokio::join!(
            spawn_blocking(move || compass.decide_with_mcts(&embedding, 100)),
            erag.retrieve_top3(&embedding, 3)
        );
        let compass_result = compass_decision.map_err(|e| anyhow::anyhow!("Compass error: {}", e))?;
        let memories = erag_docs?;  // Vec<String>
        context.traversal_result = Some(json!({"decision": compass_result}));  // Stub traversal
        context.retrieved_memories = memories.iter().map(|doc| (doc.clone(), 0.8)).collect();  // Stub scores
        context.metrics.memory_count = memories.len();
        context.log_stage_time("retrieval", decision_start);  // Combined time

        // MCTS-RAG Bridge: Extract queries from low-confidence nodes
        let queries = compass.extract_retrieval_queries(&embedding, 0.8);
        if !queries.is_empty() {
            warn!("High entropy - extracted queries: {:?}", queries);
            // Additional retrieval (stub)
        }

        let full_context = format!("Compass: {}, ERAG: {:?}", compass_result, memories.join(", "));

        // STAGE 3: HYBRID GENERATION (150ms target)
        let gen_start = Instant::now();
        let generation = self.hybrid_generate(&full_context, &embedding, &compass_result, context).await?;
        context.generated_response = generation.clone();
        context.log_stage_time("generation", gen_start);

        // STAGE 4: LEARNING (fire-and-forget)
        let reward = if is_breakthrough(&generation) { 15.0 } else { 0.0 };  // Stub ROUGE
        let learning_event = LearningEvent { prompt: prompt.to_string(), generation, reward, emotional: emotional_valence };
        tokio::spawn(async move {
            // QLoRA train (stub - integrate qlora.rs)
            if let Err(e) = qlora_train_async(learning_event).await {
                error!("Learning failed: {}", e);
            }
        });

        // Final metrics & Phase 6 (existing)
        let total_ms = start.elapsed().as_millis() as f32;
        context.metrics.total_time_ms = total_ms;
        let phase6_metrics = if self.config.enable_phase6 {
            self.integrate_phase6_stage(&context.generated_response, &PipelineInput { text: prompt.to_string(), ..Default::default() }).await.ok()
        } else { None };

        let output = PipelineOutput {
            response: context.generated_response.clone(),
            consciousness_state: self.get_current_consciousness_state().await,
            processing_time_ms: total_ms,
            performance_metrics: PipelinePerformanceMetrics {
                total_latency_ms: total_ms,
                memory_usage_mb: 0.0,  // Stub - integrate sysinfo
                gpu_utilization: 0.75,  // Stub
                success_rate: 1.0,
                throughput_ops_per_sec: 1000.0 / total_ms,
                stage_breakdown: vec![],  // Populate from context
            },
            phase6_metrics,
        };

        info!("✅ NIODOO Pipeline complete: {}ms, Memories: {}", total_ms, context.metrics.memory_count);
        println!("JSON Output: {}", context.to_json_output());  // Test output

        Ok(output)
    }

    /// Hybrid Generation with Fallbacks
    async fn hybrid_generate(&self, prompt: &str, embedding: &[f32], compass: &str, context: &mut PipelineContext) -> Result<String> {
        let full_prompt = format!("{} Context: Compass={}, Embedding entropy high? {}", prompt, compass, embedding.len());

        // Cascading fallbacks with 5s timeouts
        let client = HTTP_CLIENT.clone();  // Global pooling

        // Claude first
        match timeout(Duration::from_secs(5), async {
            let res = client.post("https://api.anthropic.com/v1/messages")
                .header("x-api-key", env::var("ANTHROPIC_KEY").unwrap_or_default())
            ...
            Ok(Ok(text)) if !text.is_empty() => return Ok(text),
            _ => warn!("Claude fallback failed, trying GPT"),
        }

        // GPT fallback
        match timeout(Duration::from_secs(5), async {
            let res = client.post("https://api.openai.com/v1/chat/completions")
                .header("Authorization", format!("Bearer {}", env::var("OPENAI_KEY").unwrap_or_default()))
                .header("Content-Type", "application/json")
                .body(json!({
                    "model": "gpt-4o-mini",
                    "messages": [{"role": "user", "content": full_prompt}]
            ...
            Ok(Ok(text)) if !text.is_empty() => return Ok(text),
            _ => warn!("GPT fallback failed, using local vLLM"),
        }

        // Local vLLM (stub - integrate vllm-rs or candle)
        Ok(format!("Local vLLM generation: Emotional response based on {} context.", compass))
    }
}
...
async fn qlora_train_async(event: LearningEvent) -> Result<()> {
    // Stub - full QLoRA in learning/qlora.rs
    info!("QLoRA training: reward {} for emotional {}", event.reward, event.emotional);
    Ok(())
}

fn is_breakthrough(text: &str) -> bool {
    text.contains("breakthrough") || text.contains("insight")  // Stub
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_niodoo_process() {
        let mut context = PipelineContext::new();
        let orchestrator = ConsciousnessPipelineOrchestrator::new(Arc::new(RwLock::new(ConsciousnessEngine::new().await.unwrap())), PipelineConfig::default());
        
        let result = orchestrator.niodoo_process("test prompt", &mut context).await.unwrap();
        
        assert!(!result.response.is_empty());
        assert!(result.processing_time_ms > 0.0);
        println!("Test JSON: {}", context.to_json_output());
    }
}
