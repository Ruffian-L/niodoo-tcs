...
use crate::consciousness_compass::{
    CompassState, CompassTracker, BreakthroughMoment, StrategicAction
};
use crate::memory::guessing_spheres::GuessingMemorySystem;
use crate::qwen_curator::LearningEvent;
use crate::rag_integration::{EmotionalVector, RagEngine, RagConfig};
use crate::token_promotion::{TokenPromotionEngine, PromotionConfig, DynamicTokenizer, ConsensusEngine, PatternDiscoveryEngine, NodeId};
...
impl TrainingDataExporter {
        ...
        let vllm_bridge = if config.enable_vllm {
            if let Some(ref url) = config.vllm_url {
                ...
                match VLLMBridge::connect(url, config.vllm_api_key.clone()) {
                    ...
                    Err(e) => {
                        warn!("⚠️  Failed to connect to vLLM: {}. Using placeholder responses.", e);
                        None
                    }
                }
            } else {
        ...
        let (token_promotion_engine, memory_system) = if config.enable_dynamic_tokenizer {
            ...
            match tokenizers::Tokenizer::from_file(&tokenizer_path) {
                Ok(base_tokenizer) => {
                    ...
                    let pattern_discovery = Arc::new(PatternDiscoveryEngine::new(
                        tda_calculator,
                        spatial_hash,
                    ));
                    ...
                    let engine = TokenPromotionEngine::new(
                        pattern_discovery,
                        consensus,
                        tokenizer_arc,
                    ...
                    (Some(Arc::new(engine)), Some(Arc::new(RwLock::new(memory_system))))
                }
                Err(e) => {
                    warn!("⚠️  Failed to load base tokenizer: {}. Dynamic tokenizer disabled.", e);
                    (None, None)
                }
            }
        } else {
    ...
    pub async fn generate_training_example(
        ...
        let recalled_docs = self.rag_engine.retrieve(
            &query_emotion,
        ...
        let output = self.generate_response_with_params(&input_text, &erag_context, &emotional_state, temperature, top_p).await?;
    ...
    pub async fn export_consciousness_training_data(&mut self) -> Result<Vec<TrainingExample>> {
        info!("Starting training data export for {} samples", self.config.num_samples);
        ...
        for i in 0..self.config.num_samples {
            ...
            if i > 0 && i % promotion_interval == 0 && self.token_promotion_engine.is_some() {
                self.run_token_promotion_cycle(i / promotion_interval).await?;
            }
            ...
            if let Some(ref memory_system) = self.memory_system {
                ...
                let sphere_id = crate::memory::guessing_spheres::SphereId(format!("training_sample_{}", i));
                let position = [
            ...
            let example = self.generate_training_example(input_text, emotional_state).await?;
            ...
            if (i + 1) % 10 == 0 {
                info!("✅ Progress: {} / {} samples ({:.1}% complete)", 
                    i + 1, 
        ...
        info!("Training data export complete: {} examples", training_data.len());
        Ok(training_data)
    }
    
    /// Run a token promotion cycle and record metrics
    async fn run_token_promotion_cycle(&mut self, cycle_num: usize) -> Result<()> {
        if let (Some(ref engine), Some(ref memory_system)) = (&self.token_promotion_engine, &self.memory_system) {
            ...
            let avg_promotion_score = if !result.promoted.is_empty() {
                result.promoted.iter().map(|t| t.promotion_score).sum::<f64>() / result.promoted.len() as f64
            } else {
                0.0
            };
            ...
            info!(
                ...
                cycle_num, oov_rate, metric.vocab_size, metric.promoted_count, 
                metric.pruned_count, avg_promotion_score
            );
    ...
    pub fn save_to_file(&mut self, output_path: PathBuf) -> Result<()> {
        ...
        self.export_learning_curve_with_compass(output_path.parent().unwrap().join("learning_curve_compass.csv"))?;

        let json = serde_json::to_string_pretty(&self.examples)?;
        std::fs::write(&output_path, json)?;
        info!("Saved {} training examples to {:?}", self.examples.len(), output_path);
        Ok(())
    }
    ...
    pub fn save_learning_curve(&self, output_path: PathBuf) -> Result<()> {
        ...
        let mut csv_content = String::from("cycle,oov_rate,vocab_size,promoted_count,pruned_count,avg_promotion_score,timestamp\n");
        ...
        info!("Saved {} learning curve metrics to {:?}", self.learning_curve.len(), output_path);
        Ok(())
    }
    ...
    fn calculate_entropy_after(&self, context: &[String]) -> f32 {
        ...
        let unique_tokens: std::collections::HashSet<_> = context
            .iter()
    ...
    fn generate_consciousness_event(&self, index: usize) -> Result<(String, EmotionalVector)> {
        ...
        let concepts = vec![
            ...
            ("experiencing joy in learning a new concept", EmotionalVector { joy: 0.8, sadness: 0.1, anger: 0.0, fear: 0.1, surprise: 0.3 }),
            ("discovering unexpected patterns in data", EmotionalVector { joy: 0.6, sadness: 0.1, anger: 0.0, fear: 0.2, surprise: 0.9 }),
            ("achieving breakthrough understanding", EmotionalVector { joy: 0.9, sadness: 0.0, anger: 0.0, fear: 0.0, surprise: 0.7 }),
            
            // Loss & Grief (Sadness + Fear dominant)
            ("processing loss and grief", EmotionalVector { joy: 0.1, sadness: 0.9, anger: 0.2, fear: 0.3, surprise: 0.1 }),
            ("reflecting on missed opportunities", EmotionalVector { joy: 0.2, sadness: 0.7, anger: 0.1, fear: 0.4, surprise: 0.0 }),
            ("experiencing disappointment after failure", EmotionalVector { joy: 0.0, sadness: 0.8, anger: 0.3, fear: 0.5, surprise: 0.2 }),
            
            // Conflict & Anger (Anger dominant)
            ("analyzing conflict resolution strategies", EmotionalVector { joy: 0.2, sadness: 0.3, anger: 0.7, fear: 0.4, surprise: 0.2 }),
            ("processing frustration with obstacles", EmotionalVector { joy: 0.1, sadness: 0.2, anger: 0.8, fear: 0.3, surprise: 0.1 }),
            ("confronting injustice and unfairness", EmotionalVector { joy: 0.0, sadness: 0.4, anger: 0.9, fear: 0.2, surprise: 0.3 }),
            
            // Fear & Anxiety (Fear + Sadness)
            ("exploring unknown territories with caution", EmotionalVector { joy: 0.3, sadness: 0.4, anger: 0.1, fear: 0.8, surprise: 0.5 }),
            ("facing uncertainty about the future", EmotionalVector { joy: 0.1, sadness: 0.5, anger: 0.2, fear: 0.9, surprise: 0.3 }),
            ("contemplating existential questions", EmotionalVector { joy: 0.2, sadness: 0.6, anger: 0.0, fear: 0.7, surprise: 0.4 }),
            
            // Curiosity & Wonder (Joy + Surprise)
            ("wondering about the nature of consciousness", EmotionalVector { joy: 0.7, sadness: 0.0, anger: 0.0, fear: 0.2, surprise: 0.8 }),
            ("exploring creative possibilities", EmotionalVector { joy: 0.8, sadness: 0.0, anger: 0.0, fear: 0.1, surprise: 0.7 }),
            ("investigating mysterious phenomena", EmotionalVector { joy: 0.6, sadness: 0.1, anger: 0.0, fear: 0.3, surprise: 0.9 }),
            
            // Mixed States (Multiple emotions balanced)
            ("processing complex moral dilemmas", EmotionalVector { joy: 0.3, sadness: 0.4, anger: 0.3, fear: 0.5, surprise: 0.3 }),
            ("experiencing bittersweet memories", EmotionalVector { joy: 0.5, sadness: 0.6, anger: 0.0, fear: 0.2, surprise: 0.4 }),
            ("balancing hope and anxiety about change", EmotionalVector { joy: 0.6, sadness: 0.3, anger: 0.1, fear: 0.7, surprise: 0.5 }),
            
            // Equilibrium States (Approaching 2-bit convergence)
            ("achieving emotional equilibrium", EmotionalVector { joy: 0.5, sadness: 0.3, anger: 0.2, fear: 0.3, surprise: 0.4 }),
            ("experiencing calm introspection", EmotionalVector { joy: 0.4, sadness: 0.3, anger: 0.1, fear: 0.3, surprise: 0.3 }),
        ];
    ...
    pub fn consolidate_breakthroughs(&mut self) -> Result<()> {
        info!("💾 Consolidating {} breakthrough moments into ERAG memory",
        ...
        for breakthrough in &self.breakthrough_moments {
            ...
            if let Some(ref resolution) = breakthrough.resolution_action {
                ...
                debug!("  ✓ Stored breakthrough: {} (reward: {:.2})",
    ...
    pub fn get_stats(&self) -> TrainingDataStats {
        let avg_entropy_before: f32 = self.examples.iter()
            .map(|e| e.entropy_before)
            .sum::<f32>() / self.examples.len() as f32;
        
        let avg_entropy_after: f32 = self.examples.iter()
            .map(|e| e.entropy_after)
            .sum::<f32>() / self.examples.len() as f32;
        
        let avg_context_size: f32 = self.examples.iter()
            .map(|e| e.erag_context.len())
            .sum::<usize>() as f32 / self.examples.len() as f32;
...
impl std::fmt::Display for TrainingDataStats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            (1.0 - (self.avg_entropy_after - self.target_entropy).abs() / self.target_entropy) * 100.0
        )
    }
}