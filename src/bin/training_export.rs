...
async fn main() -> Result<()> {
    ...
    info!("  - Target entropy: {:.2} bits", export_config.target_entropy);
    ...
    // Save to file
    let output_path = base_dir.join("training_data").join("consciousness_training_data.json");
    ...
    info!("   Entropy convergence: {:.1}%", 
        (1.0 - (stats.avg_entropy_after - stats.target_entropy).abs() / stats.target_entropy) * 100.0
    );
